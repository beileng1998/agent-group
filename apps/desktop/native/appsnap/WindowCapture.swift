    ...
    if application.bundleIdentifier == excludedBundleIdentifier {
        return .failure(
            AppSnapFailure(
                ...
                message: "Agent Group cannot capture its own window."
            )
        )
    }
...
final class OneFrameWindowCapture: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let outputQueue = DispatchQueue(label: "dev.agent-group.appsnap.stream-output")
...
final class AppSnapCaptureCoordinator {
    ...
    private let queue = DispatchQueue(label: "dev.agent-group.appsnap.capture")
