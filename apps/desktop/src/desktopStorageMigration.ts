...
export function validateAgentGroupStorageSnapshot(
  value: unknown,
): AgentGroupStorageSnapshot | null {
  ...
  try {
    if (
      Buffer.byteLength(JSON.stringify(snapshot), "utf8") > AGENT_GROUP_STORAGE_SNAPSHOT_MAX_BYTES
    ) {
      return null;
    }
  } catch {
...
export function readAgentGroupStorageSnapshot(
  snapshotPath: string,
): AgentGroupStorageSnapshot | null {
  try {
