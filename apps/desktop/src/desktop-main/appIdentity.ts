...
export function resolveAboutCommitHash(): string | null {
  ...
  try {
    ...
    ) as { agentGroupCommitHash?: unknown; agentGroupCommitHash?: unknown };
    desktopState.aboutCommitHashCache = normalizeCommitHash(
      parsed.agentGroupCommitHash ?? parsed.agentGroupCommitHash,
    );
  } catch {
    desktopState.aboutCommitHashCache = null;
  }
