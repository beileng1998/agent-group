...
import { AGENT_GROUP_DEVELOPMENT_BUNDLE_ID } from "@agent-group/shared/desktopIdentity";
...
describe("desktop AppSnap platform state", () => {
  it("exposes an explicit unsupported state outside macOS", async () => {
    ...
    const manager = new DesktopAppSnapManager({
      platform: "win32",
      helperPath: "C:\\missing\\agent-group-appsnap-helper.exe",
      captureDirectory: "C:\\tmp\\appsnap",
      excludedBundleId: AGENT_GROUP_DEVELOPMENT_BUNDLE_ID,
      onState,
      onCaptured: vi.fn(),
      onError: vi.fn(),
    });
  ...
  it("preserves a missing-helper error instead of reporting a permission problem", async () => {
    const manager = new DesktopAppSnapManager({
      platform: "darwin",
      helperPath: "/tmp/agent-group-appsnap-helper-that-does-not-exist",
      captureDirectory: "/tmp/agent-group-appsnap-test",
      excludedBundleId: AGENT_GROUP_DEVELOPMENT_BUNDLE_ID,
      onState: vi.fn(),
      onCaptured: vi.fn(),
      onError: vi.fn(),
...
describe("AppSnap helper protocol", () => {
  ...
  it("coalesces concurrent listener reconciliation while the capture directory is prepared", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "agent-group-appsnap-reconcile-"));
  ...
  it("retains a full composer batch and reports any overflow", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "agent-group-appsnap-test-"));
    const checkChild = createFakeChildProcess();
    const watchChild = createFakeChildProcess();
    const spawn = vi
      .fn()
  ...
  it("keeps the helper capture file when persisting the pending copy fails", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "agent-group-appsnap-persist-fail-"));
    const checkChild = createFakeChildProcess();
    const watchChild = createFakeChildProcess();
    const spawn = vi
      .fn()
  ...
  it("restores pending captures after a manager restart and removes them only after ack", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "agent-group-appsnap-pending-"));
    const checkChild = createFakeChildProcess();
    const watchChild = createFakeChildProcess();
    const spawn = vi
      .fn()
  ...
  it("recovers a helper PNG left behind before pending metadata was persisted", async () => {
    const captureDirectory = mkdtempSync(join(tmpdir(), "agent-group-appsnap-helper-recovery-"));
