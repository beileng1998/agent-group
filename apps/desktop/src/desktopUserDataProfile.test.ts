...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "agent-group-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Agent Group profile names", () => {
    ...
    expect(resolveDesktopUserDataPath({ appDataBase, isDevelopment: true })).toBe(
      "/Users/tester/Library/Application Support/agent-group-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, isDevelopment: false })).toBe(
      "/Users/tester/Library/Application Support/agent-group",
    );
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "agent-group");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "agent-group-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "agent-group-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Agent Group profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Agent Group database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "agent-group");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".agent-group-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "agent-group");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "agent-group-profile-seed.json"), "{");
