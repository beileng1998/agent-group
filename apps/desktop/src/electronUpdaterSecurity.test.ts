...
describe("electronUpdaterSecurity", () => {
  ...
  it("parses distinguished names the same way as builder-util-runtime", () => {
    const parsed = parseDistinguishedName('CN=Agent Group, O="Acme, Inc.", OU=Tools\\2C Desktop');

    expect(parsed.get("CN")).toBe("Agent Group");
  ...
  it("validates a matching full distinguished name with shell-free execFile options", async () => {
    const execFile = vi.fn((file, args, options, callback) => {
      callback(
        ...
        JSON.stringify({
          Status: 0,
          Path: "C:\\Users\\test\\AppData\\Local\\Temp\\AgentGroupSetup.exe",
          SignerCertificate: {
            Subject: "CN=Agent Group, O=Acme Tools",
          },
        }),
        "",
      );
    ...
    const result = await verifyWindowsUpdateCodeSignature(
      ["CN=Agent Group, O=Acme Tools"],
      "C:\\Users\\test\\AppData\\Local\\Temp\\AgentGroupSetup.exe",
  ...
  it("keeps the upstream CN-only fallback warning", async () => {
      ...
      logger,
      {
        ...
        execFile: vi.fn((_file, _args, _options, callback) => {
          callback(
            ...
            JSON.stringify({
              Status: 0,
              Path: "C:\\Temp\\AgentGroupSetup.exe",
              SignerCertificate: { Subject: "CN=Agent Group, O=Acme Tools" },
            }),
            "",
          );
        }),
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining("Signature validated using only CN Agent Group"),
    );
  });
  ...
  it("returns a mismatch summary for an unexpected publisher", async () => {
    ...
    expect(result).toContain("publisherNames: CN=Agent Group, O=Acme Tools");
  ...
  it("patches electron-updater BaseUpdater spawnSyncLog only on Windows", () => {
    ...
    const prototype = FakeBaseUpdater.prototype as {
      ...
      __agentGroupSpawnSyncLogPatched?: boolean;
    };
    ...
    expect(prototype.__agentGroupSpawnSyncLogPatched).toBe(true);
  });
