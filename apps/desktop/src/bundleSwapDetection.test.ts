...
describe("isWatchableBundlePath", () => {
  it("accepts a packaged asar app path", () => {
    expect(isWatchableBundlePath("/Applications/Agent Group.app/Contents/Resources/app.asar")).toBe(
      true,
    );
  });

  it("rejects an unpackaged directory app path", () => {
    expect(isWatchableBundlePath("/Users/me/dev/agent-group/apps/desktop")).toBe(false);
  });
});
