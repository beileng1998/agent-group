...
describe("resolveLaunchVersionRecordPath", () => {
  it("places the record file inside the userData directory", () => {
    expect(resolveLaunchVersionRecordPath("/home/me/AppData/Agent Group")).toBe(
      "/home/me/AppData/Agent Group/last-launch-version.json",
    );
  });
});
...
describe("resolveMacAppBundlePath", () => {
  it("resolves the .app bundle from the Electron executable on macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Agent Group.app/Contents/MacOS/Agent Group", "darwin"),
    ).toBe("/Applications/Agent Group.app");
  });

  it("returns null off macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Agent Group.app/Contents/MacOS/Agent Group", "linux"),
    ).toBeNull();
  });

  it("returns null when the executable is not inside a .app bundle", () => {
    expect(resolveMacAppBundlePath("/usr/local/bin/agent-group", "darwin")).toBeNull();
  });
});
