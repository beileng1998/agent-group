...
import type { BrowserExecuteCdpInput, ThreadBrowserState, ThreadId } from "@agent-group/contracts";
...
const BROWSER_USE_PIPE_NAME_PREFIX = "agent-group-iab";
export const AGENT_GROUP_BROWSER_USE_PIPE_ENV = "AGENT_GROUP_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserUsePipePath(
  ...
  const configured = env[AGENT_GROUP_BROWSER_USE_PIPE_ENV]?.trim();
  return configured || resolveDefaultBrowserUsePipePath(platform);
}

export const AGENT_GROUP_BROWSER_USE_PIPE_PATH = resolveConfiguredBrowserUsePipePath();
...
export class BrowserUsePipeServer {
  ...
  constructor(
    ...
    options: BrowserUsePipeServerOptions | string = AGENT_GROUP_BROWSER_USE_PIPE_PATH,
  ) {
    this.pipePath =
      typeof options === "string" ? options : (options.pipePath ?? AGENT_GROUP_BROWSER_USE_PIPE_PATH);
  ...
  private async handleRequest(method: string, params: unknown): Promise<unknown> {
    switch (method) {
      ...
      case "getInfo":
        const sessionId = asString(asObject(params)?.session_id);
        return {
          name: "Agent Group In-app Browser",
          version: "0.1.0",
          type: "iab",
  ...
  private async createTabForSession(sessionId: string): Promise<{
    ...
    if (!snapshot) {
      throw new Error("No active Agent Group browser pane available");
    }
