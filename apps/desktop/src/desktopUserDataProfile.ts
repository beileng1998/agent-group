...
// Purpose: Resolves Agent Group's Electron userData paths and completes bridge profile repair.

import * as FS from "node:fs";
import * as OS from "node:os";
import * as Path from "node:path";

const DEV_USER_DATA_DIR_NAME = "agent-group-dev";
const PROD_USER_DATA_DIR_NAME = "agent-group";
const BRIDGE_PROFILE_MANIFEST_FILE_NAME = "agent-group-profile-seed.json";
const CANONICAL_BROWSER_PARTITION_NAME = "agent-group-browser";
 ...
 *
 * The bridge manifest identifies the exact sibling profile that supplied the Agent Group profile.
...
export function repairBrowserProfileFromBridgeManifest(
  ...
  try {
    ...
    for (const entryGroup of BROWSER_PARTITION_SEED_ENTRY_GROUPS) {
      ...
      const stagedGroupPath = FS.mkdtempSync(Path.join(targetPartitionPath, ".agent-group-bridge-"));
