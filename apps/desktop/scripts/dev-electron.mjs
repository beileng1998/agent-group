...
function cleanupStaleDevApps() {
  ...
  spawnSync("pkill", ["-f", "--", `--agent-group-dev-root=${desktopDir}`], { stdio: "ignore" });
}
...
function listStaleComputerUsePids() {
  ...
  const result = spawnSync("pgrep", ["-fal", "Agent Group \\(Dev\\).*(computerUseMcp\\.mjs mcp)"], {
    encoding: "utf8",
  });
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Agent Group (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const result = spawnSync(
    "pgrep",
    ["-fal", "/Applications/Agent Group\\.app/Contents/MacOS/Agent Group"],
    {
      encoding: "utf8",
    },
  );
  ...
  console.error(
    "[desktop-dev] Agent Group is still running. Close it before testing voice in Agent Group (Dev), or you may be looking at the wrong app/runtime.",
  );
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--agent-group-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
