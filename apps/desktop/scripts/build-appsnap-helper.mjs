...
function buildFingerprint({ arch, release, sources, targets }) {
  ...
  hash.update("agent-group-appsnap-helper-build-v1\0");
  hash.update(arch);
  hash.update("\0");
...
export function buildAppSnapHelper({
  ...
  const temporaryDirectory = mkdtempSync(join(tmpdir(), "agent-group-appsnap-helper-"));
  ...
  try {
    ...
    for (const target of targets) {
      const thinBinary = join(temporaryDirectory, `agent-group-appsnap-helper-${target.arch}`);
    ...
    const unsignedBinary = join(temporaryDirectory, "agent-group-appsnap-helper");
