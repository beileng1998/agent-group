// FILE: vite.config.ts
// Purpose: Builds the Agent Group web client and controls diagnostic source maps.
...
const sourcemapEnv = process.env.AGENT_GROUP_WEB_SOURCEMAP?.trim().toLowerCase();

function resolveBackendHttpOrigin(rawWsUrl: string | undefined): string | undefined {
...
const backendHttpOrigin = resolveBackendHttpOrigin(process.env.VITE_WS_URL);
...
function centralIconPrunePlugin(): Plugin {
  ...
  return {
    name: "agent-group-central-icon-prune",
    apply: "build",
...
export default defineConfig({
  ...
  server: {
    port,
    strictPort: true,
    // Browser auth endpoints are same-origin URLs. In development the page is
    // served by Vite while the isolated Agent Group backend owns `/api`, so proxy
    // those requests instead of letting Vite's SPA fallback masquerade as JSON.
    ...(backendHttpOrigin
      ? {
          proxy: {
            "/api": {
