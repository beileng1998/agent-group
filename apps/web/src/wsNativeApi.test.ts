...
describe("wsNativeApi", () => {
  it("routes browser folder picks to the host system dialog", async () => {
    requestMock.mockResolvedValue("/Users/tester/project");
    Reflect.deleteProperty(getWindowForTest(), "desktopBridge");
    ...
    expect(requestMock).toHaveBeenCalledWith("dialogs.pickFolder", {});
    expect(picked).toBe("/Users/tester/project");
  });
  ...
  it("delivers and caches valid server.welcome payloads", async () => {
    ...
    const payload = { cwd: "/tmp/workspace", homeDir: "/Users/tester", projectName: "agent-group-code" };
  ...
  it("delivers and caches server settings updates", async () => {
    ...
    const payload = {
      settings: {
        ...
        remoteAccess: { enabled: false, hostname: "agent-group" },
        skills: { disabled: [] },
      },
    } as const;
  ...
  it("forwards valid terminal and git progress events", async () => {
    const { createWsNativeApi } = await import("./wsNativeApi");
