...
export const AGENT_GROUP_WS_TRANSPORT_STATE_EVENT = "agent-group:ws-transport-state";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(AGENT_GROUP_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(AGENT_GROUP_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(AGENT_GROUP_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
