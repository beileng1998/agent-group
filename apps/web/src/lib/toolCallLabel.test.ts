...
describe("deriveReadableCommandDisplay", () => {
  ...
  it("removes env and timeout wrappers from inline command summaries", () => {
    expect(
      deriveReadableCommandDisplay(
        "env -u AGENT_GROUP_AUTH_TOKEN AGENT_GROUP_PORT_OFFSET=3158 timeout 180s bun run dev",
        true,
      ),
    ).toEqual({
      verb: "Running",
      target: "bun run dev",
      fullCommand: "env -u AGENT_GROUP_AUTH_TOKEN AGENT_GROUP_PORT_OFFSET=3158 timeout 180s bun run dev",
    });
  });
