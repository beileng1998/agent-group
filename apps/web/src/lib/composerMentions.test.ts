...
describe("composer mention reference filtering", () => {
  it("does not invent plugin references for plain file or folder mentions", () => {
    expect(filterPromptMentionReferences("Open @Things please", [])).toEqual([]);
  });
  ...
  it("preserves selected plugin references only while their token remains in the prompt", () => {
    ...
    expect(
      filterPromptMentionReferences("Open @Things please", [thingsPlugin, githubPlugin]),
  ...
  it("drops selected plugin references after the matching token is removed", () => {
    ...
    expect(filterPromptMentionReferences("Open @src/things please", [thingsPlugin])).toEqual([]);
  });
  ...
  it("matches quoted plugin mention tokens when the plugin name contains whitespace", () => {
    ...
    expect(filterPromptMentionReferences('Use @"Google Drive" please', [plugin])).toEqual([plugin]);
  });
  ...
  it("matches plugin mention tokens from plugin:// paths when display names differ", () => {
    ...
    expect(filterPromptMentionReferences("Use @linear please", [plugin])).toEqual([plugin]);
  });
  ...
  it("preserves and renders session mentions by their selected display token", () => {
    const session = {
      kind: "session" as const,
      sessionId: ThreadId.makeUnsafe("thread-research"),
      name: "Research notes",
    };

    expect(filterPromptMentionReferences('Use @"Research notes"', [session])).toEqual([session]);
    expect(resolveMentionChipKind("Research notes", { mentionReferences: [session] })).toBe(
      "session",
    );
    expect(filterPromptMentionReferences("Use something else", [session])).toEqual([]);
  });
