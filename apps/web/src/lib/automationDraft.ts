...
import { DEFAULT_AUTOMATION_FAST_INTERVAL_MAX_ITERATIONS } from "@agent-group/contracts";
import type {
  AutomationMode,
  AutomationSchedule,
...
export function buildAutomationDraftWarnings(input: {
    ...
    warnings.push({
      ...
      detail:
        input.worktreeMode === "auto"
          ? "If Agent Group cannot create a worktree, runs may fall back to editing the active project checkout."
  ...
  if (
    ...
    warnings.push({
      ...
      detail: "Agent Group was not fully confident about the parsed automation fields.",
      requiresAcknowledgement: false,
    });
  }
