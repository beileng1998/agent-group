...
describe("resolveWorkspaceFileOpenTarget", () => {
  ...
  it("maps Agent Group public asset URLs to their workspace files", () => {
    expect(
...
describe("resolveScratchPreviewFileOpenTarget", () => {
  const scratchPdf = "/private/tmp/agent-group-codex-workspaces/thread-1/report.pdf";
  ...
  it("returns absolute scratch-workspace preview paths unchanged", () => {
    ...
    expect(
      resolveScratchPreviewFileOpenTarget("/tmp/agent-group-codex-workspaces/thread-1/shot.png"),
    ).toBe("/tmp/agent-group-codex-workspaces/thread-1/shot.png");
  });
  ...
  it("returns null for scratch-workspace files without an in-app binary preview", () => {
    expect(
      resolveScratchPreviewFileOpenTarget("/tmp/agent-group-codex-workspaces/thread-1/notes.ts"),
    ).toBeNull();
  });
  ...
  it("returns null for relative paths", () => {
    ...
    expect(
      resolveScratchPreviewFileOpenTarget("agent-group-codex-workspaces/thread-1/a.pdf"),
    ).toBeNull();
  });
});
