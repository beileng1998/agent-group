...
export const serverQueryKeys = {
  ...
  agentGroupOverview: (groupId: ProjectId) => ["server", "agent-group", groupId] as const,
...
export function agentGroupOverviewQueryOptions(groupId: ProjectId) {
  return queryOptions({
    queryKey: serverQueryKeys.agentGroupOverview(groupId),
    queryFn: () => ensureNativeApi().agentGroup.getOverview({ groupId }),
    staleTime: 5_000,
  });
}
...
// through lsof/ps; active Agent Group-owned runs still poll for responsive status.
...
// UTC offset; all metrics are computed from Agent Group's local DB projections.
