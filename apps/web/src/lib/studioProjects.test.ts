...
import { type ProjectId, type ThreadId } from "@agent-group/contracts";
...
function makeProject(overrides: Partial<Project> = {}): Project {
  return {
    ...
    folderName: "Studio",
    localName: null,
    cwd: "/Users/tester/Documents/Agent Group/Studio",
    defaultModelSelection: null,
    expanded: false,
    scripts: [],
    ...overrides,
...
describe("studioProjects", () => {
  ...
  it("matches the configured Studio root and nested Studio paths", () => {
    ...
    expect(
      isStudioContainerProject(
        makeProject({ cwd: "/Users/tester/Documents/Agent Group/Studio/Outbox" }),
        paths,
      ),
    ).toBe(true);
  });
  ...
  it("prefers the canonical Studio root container over nested studio-kind rows", () => {
    const nestedStudioProject = makeProject({
      ...
      cwd: "/Users/tester/Documents/Agent Group/Studio/Outbox",
    });
  ...
  it("recovers and hydrates the existing Studio project when the server rejects a duplicate create", async () => {
    ...
    nativeApiMock.dispatchError = new Error(
      "Orchestration command invariant failed (project.create): Project 'project-server-studio' already uses workspace root '/Users/tester/Documents/Agent Group/Studio'.",
    );
    nativeApiMock.shellSnapshotProjects = [
      {
  ...
  it("retries duplicate Studio recovery while the shell snapshot catches up", async () => {
    ...
    nativeApiMock.dispatchError = new Error(
      "Orchestration command invariant failed (project.create): Project 'project-retried-studio' already uses workspace root '/Users/tester/Documents/Agent Group/Studio'.",
    );
    nativeApiMock.shellSnapshotProjectBatches = [
      [],
      [
