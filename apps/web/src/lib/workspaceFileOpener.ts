...
import { isSupportedLocalPreviewFilePath } from "@agent-group/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
...
const AGENT_GROUP_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const AGENT_GROUP_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveAgentGroupPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!AGENT_GROUP_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${AGENT_GROUP_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveAgentGroupPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
