...
describe("browserDownload", () => {
  ...
  it("prefers the server filename from Content-Disposition", async () => {
    globalThis.fetch = vi.fn(() =>
      Promise.resolve(
        new Response("zip", {
          status: 200,
          headers: {
            "Content-Disposition": 'attachment; filename="agent-group-thread-pretty.zip"',
          },
        }),
      ),
    );
    ...
    await downloadUrlAsBlob({
      ...
      filename: "agent-group-thread-thread-1.zip",
    });

    expect(link.download).toBe("agent-group-thread-pretty.zip");
    expect(click).toHaveBeenCalledTimes(1);
  });
  ...
  it("falls back to the caller filename when Content-Disposition is malformed", async () => {
    ...
    expect(link.download).toBe("agent-group-thread-thread-1.zip");
    expect(click).toHaveBeenCalledTimes(1);
  });
