...
describe("threadDetailSubscriptionRetention", () => {
  ...
  it("subscribes retained dock threads before their shell row exists", () => {
    const promotedThreadId = ThreadId.makeUnsafe("promoted-sidechat");
    const nestedSidechatId = ThreadId.makeUnsafe("nested-sidechat");

    expect(
      resolveThreadDetailSubscriptionIds({
        visibleThreadIds: [promotedThreadId],
        retainedThreadIds: [nestedSidechatId],
      }),
    ).toEqual([promotedThreadId, nestedSidechatId]);
  });
