...
import { getAgentMentionAutocompleteAliases } from "@agent-group/contracts";
...
import {
  ...
  type AgentGroupSessionMentionCandidate,
...
const EMPTY_SESSION_MENTIONS: readonly AgentGroupSessionMentionCandidate[] = [];
...
export function useComposerCommandMenuItems(input: {
  ...
  const {
    ...
    dynamicAgents,
    sessionMentions = EMPTY_SESSION_MENTIONS,
  } = input;
  ...
  return useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "mention") {
      ...
      const sessionItems: ComposerCommandItem[] = rankAgentGroupSessionMentionCandidates(
        sessionMentions,
        query,
      ).map((session) => ({
        ...
        title: session.title,
        mentionName: session.mentionName,
        label: session.title,
      ...
      // Session coordination is the product-level intent; provider plugins,
      // local context, and subagent delegation remain available below it.
      return [...sessionItems, ...pluginItems, ...localRootItems, ...pathItems, ...agentItems];
    }
