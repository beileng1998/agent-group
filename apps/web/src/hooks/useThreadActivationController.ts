...
type SidebarThreadActivationSummary = Pick<
  ...
  "id" | "projectId" | "parentThreadId" | "forkSourceThreadId" | "sidechatSourceThreadId"
>;
...
function resolveSidechatSplitActivation(
  ...
  if (!options.targetThread || !isTemporarySidechatThread(options.targetThread)) {
    return null;
  }
  const sourceThreadId = options.targetThread.sidechatSourceThreadId;
  if (!sourceThreadId) {
    return null;
  }
  const sourceThread = input.sidebarThreadSummaryById[sourceThreadId];
