...
describe("mergeDynamicModelOptions", () => {
  ...
  it("keeps resolved Claude custom model slugs selectable", () => {
    const options = mergeDynamicModelOptions({
      provider: "claudeAgent",
      staticOptions: [],
      dynamicModels: [
        { slug: "kimi-k3", name: "kimi-k3" },
        { slug: "doubao-seed-evolving", name: "doubao-seed-evolving" },
      ],
    });

    expect(options.map((option) => option.slug).toSorted()).toEqual([
