...
describe("splitViewStore", () => {
  ...
  it("keeps writing split views to the Agent Group v1 storage key so persisted state can migrate", async () => {
    vi.resetModules();
    globalThis.localStorage = createMemoryStorage();
    ...
    const persisted = globalThis.localStorage.getItem("agent-group:split-view-state:v1");
    expect(persisted).not.toBeNull();
    expect(globalThis.localStorage.getItem("agent-group:split-view-state:v2")).toBeNull();
  ...
  it("migrates legacy v1 flat split views from the stable storage key", async () => {
    ...
    globalThis.localStorage.setItem(
      "agent-group:split-view-state:v1",
      JSON.stringify({
        state: {
          splitViewsById: {
            "split-legacy": {
