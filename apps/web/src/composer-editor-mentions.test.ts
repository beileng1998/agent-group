...
describe("splitPromptIntoDisplaySegments", () => {
  ...
  it("converts a trailing URL into a link segment for read-only rendering", () => {
    expect(
      splitPromptIntoDisplaySegments("https://github.com/example/project/pull/155"),
    ).toEqual([{ type: "link", url: "https://github.com/example/project/pull/155" }]);
  });
  ...
  it("renders a URL on its own line followed by trailing prose", () => {
    expect(
      splitPromptIntoDisplaySegments(
        "https://github.com/example/project/pull/155\nfix the conflicts",
      ),
    ).toEqual([
      { type: "link", url: "https://github.com/example/project/pull/155" },
      { type: "text", text: "\nfix the conflicts" },
    ]);
  });
  ...
  it("uses explicit mention references instead of inferring plugins from plain @text", () => {
    ...
    expect(
      splitPromptIntoDisplaySegments('Use @"Research notes"', [
        {
          kind: "session",
      ...
      { type: "text", text: "Use " },
      { type: "mention", path: "Research notes", kind: "session" },
    ]);
  });
});
