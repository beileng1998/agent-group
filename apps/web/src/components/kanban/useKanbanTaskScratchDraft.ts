...
import type { ModelSlug, ProviderKind } from "@agent-group/contracts";
...
export function useKanbanTaskScratchDraft(input: { readonly defaultProvider: ProviderKind }) {
  ...
  useEffect(() => {
    const nextMentions = filterPromptMentionReferences(prompt, composerMentions);
    if (!mentionReferencesEqual(composerMentions, nextMentions)) {
