...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.agent-group/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.agent-group/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .agent-group worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.agent-group/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("syncs stale local thread metadata to the concrete git checkout", () => {
    expect(
      shouldSyncLocalThreadBranch({
        envMode: "local",
        activeWorktreePath: null,
        activeThreadBranch: "agent-group/pi",
        currentGitBranch: "main",
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/agent-group-pi",
        existingAssociatedWorktreeBranch: "agent-group/pi",
        existingAssociatedWorktreeRef: "agent-group/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/agent-group-pi",
      associatedWorktreeBranch: "agent-group/pi",
      associatedWorktreeRef: "agent-group/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.agent-group/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.agent-group/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.agent-group/worktrees/feature-b",
      nextWorktreePath: "/repo/.agent-group/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.agent-group/worktrees/feature-a",
      nextWorktreePath: "/repo/.agent-group/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
