...
// catalog (~/.agent-group/skills plus each provider's skills folder), shows which provider
...
export function SkillsSettingsPanel() {
  ...
  const agentGroupSkillsDir = catalogQuery.data?.agentGroupSkillsDir;

  return (
    <div className="space-y-8">
      <SettingsSection title="Portable skills">
        <SettingsRow
          title="Agent Group skills folder"
          description="Skills placed here are available on every provider. When a provider already ships its own copy of a skill, that copy is used; otherwise Agent Group's copy is the fallback."
          status={
            agentGroupSkillsDir ? (
              <code className="break-all text-[11px] text-muted-foreground">{agentGroupSkillsDir}</code>
            ) : null
          }
          control={
      ...
      {catalogQuery.isError ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Agent Group could not scan the skill folders. Retry after checking that the server is running."
          />
        </SettingsSection>
      ) : null}
      ...
      {!catalogQuery.isLoading && !catalogQuery.isError && totalSkills === 0 ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Add a skill folder containing a SKILL.md to the Agent Group skills folder above, or install skills for any supported provider."
          />
        </SettingsSection>
      ) : null}
