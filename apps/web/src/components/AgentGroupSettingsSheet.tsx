...
export default function AgentGroupSettingsSheet() {
  ...
  const [contextTemplateId, setContextTemplateId] =
    useState<AgentGroupContextTemplateId | null>(null);
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      setBrowserAccess(next.browserToolsEnabled);
    ...
    <Sheet open={Boolean(groupId && project)} onOpenChange={(open) => !open && void requestClose()}>
      <SheetPopup side="right" className="w-[min(94vw,680px)] max-w-[680px]" keepMounted>
        <SheetHeader>
          ...
          <SheetDescription>
            {canonicalName}
            {project?.cwd ? <span className="mt-1 block truncate font-mono text-xs">{project.cwd}</span> : null}
          </SheetDescription>
        </SheetHeader>
        ...
        <SheetPanel className="pb-8">
          {loading && !config ? (
            ...
            <div className="space-y-6">
                ...
                <SettingsRow
                  ...
                  control={
                    <div className="flex w-full gap-2 sm:w-72">
                      ...
                      />
                      <Button size="sm" disabled={!nameDirty || nameSaveState === "saving"} onClick={() => void saveName()}>
                        Rename
                      </Button>
                    </div>
                  }
              ...
              <SettingsSection title="Advanced">
                <SettingsRow
                  ...
                  control={
                    <Button variant="outline" size="sm" onClick={() => void openGlobalPromptSettings()}>
            ...
            <div className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
              ...
              <div className="mt-3">
                <Button variant="outline" size="sm" onClick={() => void loadConfig()}>Retry</Button>
              </div>
            </div>
          )}

          {error && config ? (
            <p className="mt-4 rounded-md bg-destructive/8 px-3 py-2 text-xs text-destructive">{error}</p>
          ) : null}
        </SheetPanel>

        <SheetFooter>
          <Button variant="ghost" onClick={() => void requestClose()}>Close</Button>
          <Button disabled={!configDirty || configSaveState === "saving"} onClick={() => void saveConfig()}>
            {configSaveState === "saving" ? "Saving…" : configSaveState === "saved" ? "Saved" : "Save changes"}
          </Button>
        </SheetFooter>
      </SheetPopup>
    </Sheet>
