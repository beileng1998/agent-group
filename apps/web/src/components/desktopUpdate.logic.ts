...
import type { DesktopUpdateActionResult, DesktopUpdateState } from "@agent-group/contracts";
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  const action = resolveDesktopUpdateButtonAction(state);
  if (action === "download") {
    return "This Mac has Apple Silicon, but Agent Group is still running the Intel build under Rosetta. Agent Group is preparing the native Apple Silicon update.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but Agent Group is still running the Intel build under Rosetta. Click Update to restart into the native Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but Agent Group is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
...
export function getDesktopUpdateButtonTooltip(
  ...
  if (state.errorContext === "install" && !state.downloadedVersion && state.availableVersion) {
    return `Agent Group restarted, but update ${state.availableVersion} was not installed. Click to try again.`;
  }
