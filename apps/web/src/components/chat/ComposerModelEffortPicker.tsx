...
export const ComposerModelEffortPicker = memo(function ComposerModelEffortPicker(
  ...
  const providerLabel = PROVIDER_DISPLAY_NAMES[activeProvider];
  ...
  const hiddenTriggerTitle = [
    props.hideModelLabel ? `${providerLabel} · ${modelLabel}` : null,
  ...
  const triggerButton = (
    <Button
      ...
      aria-label="Change next-turn agent and options"
          ...
          {!isMenuOpen ? (
            <TooltipPopup side="top" sideOffset={6} variant="picker">
              <span className="inline-flex items-center gap-2 px-1 py-0.5">
                <span>Change next-turn agent</span>
                <ShortcutKbd
