...
import { EnvironmentHighlightsSection } from "./EnvironmentHighlightsSection";
...
export interface EnvironmentPanelProps {
  ...
  /** Open the unified Highlights explorer for this session. */
  onViewAllHighlights: (() => void) | null;
    ...
    <div className="flex flex-col gap-0.5 p-1.5">
      ...
      */}
      ...
      (settings.showEnvironmentMarkers && threadMarkers.length > 0) ? (
        <>
          <EnvironmentSectionDivider />
          <EnvironmentHighlightsSection
            pins={settings.showEnvironmentPinned ? pinnedMessages : []}
            markers={settings.showEnvironmentMarkers ? threadMarkers : []}
            pinnedMessageTextById={pinnedMessageTextById}
            markerMessageTextById={markerMessageTextById}
            onJumpToPin={onJumpToPinnedMessage}
            onTogglePinDone={onTogglePinnedMessageDone}
            onUnpin={onUnpinMessage}
            onRenamePin={onRenamePinnedMessage}
            onJumpToMarker={onJumpToThreadMarker}
            onRemoveMarker={onRemoveThreadMarker}
            onViewAll={onViewAllHighlights}
          />
        </>
      ) : null}
