...
import type { GitPullRequestComment, PullRequestComment } from "@agent-group/contracts";
...
describe("buildResolveConflictsPrompt", () => {
  ...
  it("points at the current checkout instead of asserting the local branch name", () => {
    // Fork threads check the PR out under `agent-group/pr-N/<branch>`, so the prompt must not
