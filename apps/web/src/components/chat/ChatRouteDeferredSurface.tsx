import type { ThreadId as ThreadIdType, TurnId } from "@agent-group/contracts";
...
export function DeferredChatView(props: {
  ...
  onSidechatPromoted?: (threadId: ThreadIdType) => Promise<void> | void;
  retainThreadDetail?: boolean;
  onMounted?: () => void;
}) {
  ...
  useEffect(() => {
    if (!props.retainThreadDetail) {
      return;
    }
    return retainThreadDetailSubscription(props.threadId);
  }, [props.retainThreadDetail, props.threadId]);
  ...
  return (
    <ChatView
      ...
      {...(props.onOpenHighlights ? { onOpenHighlightsPanel: props.onOpenHighlights } : {})}
      ...
      {...(props.onSidechatPromoted ? { onSidechatPromoted: props.onSidechatPromoted } : {})}
    />
  );
}
