...
export function resolveTranscriptMarkerRange(input: {
  ...
  if (
    ...
    input.sourceRange.endOffset <= input.messageText.length
  ) {
    return input.sourceRange;
  }
...
function sourceContainerForNode(node: Node): HTMLElement | null {
  const element = node instanceof HTMLElement ? node : node.parentElement;
  return element?.closest<HTMLElement>("[data-transcript-source-start]") ?? null;
}

function sourceOffsetForBoundary(input: {
  messageContainer: HTMLElement;
  node: Node;
  offset: number;
}): number | null {
  const sourceContainer = sourceContainerForNode(input.node);
  if (!sourceContainer || !input.messageContainer.contains(sourceContainer)) return null;
  const startOffset = Number(sourceContainer.dataset.transcriptSourceStart);
  const endOffset = Number(sourceContainer.dataset.transcriptSourceEnd);
  const sourceLength = endOffset - startOffset;
  const renderedLength = sourceContainer.textContent?.length ?? 0;
  if (
    !Number.isInteger(startOffset) ||
    !Number.isInteger(endOffset) ||
    sourceLength <= 0 ||
    renderedLength !== sourceLength
  ) {
    return null;
  }
  try {
    const prefixRange = document.createRange();
    prefixRange.selectNodeContents(sourceContainer);
    prefixRange.setEnd(input.node, input.offset);
    const relativeOffset = prefixRange.toString().length;
    return relativeOffset <= sourceLength ? startOffset + relativeOffset : null;
  } catch {
    return null;
  }
}

export function resolveTranscriptSelectionSourceRange(
  range: Range,
  messageContainer: HTMLElement,
): TranscriptAssistantSelection["sourceRange"] | undefined {
  const startOffset = sourceOffsetForBoundary({
    messageContainer,
    node: range.startContainer,
    offset: range.startOffset,
  });
  const endOffset = sourceOffsetForBoundary({
    messageContainer,
...
export function readTranscriptAssistantSelection(input: {
  ...
  const sourceRange = resolveTranscriptSelectionSourceRange(range, anchorContainer);
...
export function resolveTranscriptSelectionActionLayout(input: {
  ...
  const actionWidth = input.actionWidth ?? TRANSCRIPT_SELECTION_ACTION_WIDTH_PX;
  const viewportWidth =
