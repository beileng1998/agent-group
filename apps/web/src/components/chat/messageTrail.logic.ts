...
import { type MessageId, type ThreadMarker } from "@agent-group/contracts";
import { type TimelineEntry } from "../../session-logic";

interface MessageTrailItemBase {
  id: string;
  messageId: MessageId;
  /** Whitespace-normalized, length-capped text for the hover preview. */
  preview: string;
  /** Optional whitespace-normalized, length-capped detail shown below the preview. */
  responsePreview: string;
  /** Number of attachments on the message (rendered as a small hint). */
  attachmentCount: number;
}

/** A sent-message turn anchor. */
export interface MessageTrailTurnItem extends MessageTrailItemBase {
  kind: "turn";
  id: MessageId;
...
/** A whole assistant message saved with Pin to Highlights. */
export interface MessageTrailPinItem extends MessageTrailItemBase {
  kind: "pin";
}

/** A selected assistant-message range saved as a Highlight. */
export interface MessageTrailHighlightItem extends MessageTrailItemBase {
  kind: "highlight";
  marker: ThreadMarker;
}

/** One navigable node on the left transcript rail. */
export type MessageTrailItem =
  | MessageTrailTurnItem
  | MessageTrailPinItem
  | MessageTrailHighlightItem;

export interface MessageTrailSavedItems {
  pinnedMessageIds?: ReadonlySet<MessageId> | undefined;
  threadMarkers?: readonly ThreadMarker[] | undefined;
}
...
/**
 * Project the timeline into turn anchors plus saved Pin / Highlight shortcuts, in
 * transcript order.
...
export function deriveMessageTrailItems(
  ...
  savedItems: MessageTrailSavedItems = {},
): MessageTrailItem[] {
  const items: MessageTrailItem[] = [];
  const markersByMessageId = new Map<MessageId, ThreadMarker[]>();
  for (const marker of savedItems.threadMarkers ?? []) {
    const markers = markersByMessageId.get(marker.messageId) ?? [];
    markers.push(marker);
    markersByMessageId.set(marker.messageId, markers);
  }
  for (const markers of markersByMessageId.values()) {
    markers.sort((a, b) => a.startOffset - b.startOffset || a.endOffset - b.endOffset);
  }

  // The turn object remains stable while saved nodes are inserted after assistant
  // rows. Every non-empty assistant row overwrites its response, so the last one
  // (the end-of-turn message) wins.
  let currentTurn: MessageTrailTurnItem | null = null;
  let turnOrdinal = 0;
  for (const entry of timelineEntries) {
    ...
    if (role === "user") {
      ...
      if (currentTurn && responsePreview !== "") {
        currentTurn.responsePreview = responsePreview;
      }
      if (savedItems.pinnedMessageIds?.has(entry.message.id)) {
        items.push({
          id: `pin:${entry.message.id}`,
          kind: "pin",
          messageId: entry.message.id,
          preview: responsePreview || "Pinned assistant message",
          responsePreview: "",
          attachmentCount: entry.message.attachments?.length ?? 0,
        });
      }
      for (const marker of markersByMessageId.get(entry.message.id) ?? []) {
        items.push({
          id: `highlight:${marker.id}`,
          kind: "highlight",
          messageId: entry.message.id,
          marker,
          preview: normalizePreview(marker.label || marker.selectedText),
          responsePreview: marker.note
            ? normalizePreview(marker.note)
            : marker.label
              ? normalizePreview(marker.selectedText)
              : "",
          attachmentCount: 0,
        });
      }
