...
import { type MessageId, type ThreadId, type ThreadMarker, type TurnId } from "@agent-group/contracts";
...
interface ChatTranscriptPaneProps {
  ...
  onOpenAssistantSelection?: ComponentProps<typeof MessagesTimeline>["onOpenAssistantSelection"];
...
export const ChatTranscriptPane = memo(function ChatTranscriptPane({
  ...
  // Left-edge navigation trail: turn anchors plus Pin / Highlight shortcuts in
  // transcript order. Current + visible turn anchors flow through a stable store
  // (not pane state), so scroll updates re-render only the rail.
  const trailItems = useMemo(
    () => deriveMessageTrailItems(timelineEntries, { pinnedMessageIds, threadMarkers }),
    [pinnedMessageIds, threadMarkers, timelineEntries],
  );
  ...
  const handleTrailSelect = useCallback(
    (item: MessageTrailItem) => {
      if (item.kind === "highlight") {
        timelineControllerRef?.current?.scrollToMarker(item.marker);
        return;
      }
      timelineControllerRef?.current?.scrollToMessage(item.messageId);
    },
    [timelineControllerRef],
  );
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        {agentActivityDetail && onCloseAgentActivityDetail ? (
          ...
          <MessagesTimeline
            ...
            {...(onOpenAssistantSelection ? { onOpenAssistantSelection } : {})}
