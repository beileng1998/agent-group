...
function marker(id: string, overrides: Partial<ThreadMarker> = {}): ThreadMarker {
  return {
    id: ThreadMarkerId.makeUnsafe(id),
    messageId: MessageId.makeUnsafe("a1"),
    startOffset: 2,
    endOffset: 12,
    selectedText: "saved excerpt",
    prefix: "",
    suffix: "",
    style: "highlight",
    color: "yellow",
...
describe("deriveMessageTrailItems", () => {
  it("keeps only user messages, in order, with sequential ordinals", () => {
    ...
    expect(items.map((item) => (item.kind === "turn" ? item.ordinal : null))).toEqual([1, 2]);
  });
  ...
  it("inserts pins and highlights beside their assistant message in source order", () => {
    const items = deriveMessageTrailItems(
      [
        messageEntry("u1", "user", "question"),
        messageEntry("a1", "assistant", "assistant answer"),
        messageEntry("u2", "user", "next question"),
      ],
      {
        pinnedMessageIds: new Set([MessageId.makeUnsafe("a1")]),
        threadMarkers: [
          marker("later", {
            startOffset: 18,
            endOffset: 28,
            selectedText: "later text",
            color: "pink",
          }),
          marker("earlier", {
            startOffset: 2,
            endOffset: 12,
            selectedText: "earlier text",
            label: "Decision",
            note: "Use this approach",
            color: "blue",
          }),
        ],
      },
    ...
    expect(items.map((item) => item.kind)).toEqual(["turn", "pin", "highlight", "highlight", "turn"]);
    ...
    expect(items[2]).toMatchObject({
      id: "highlight:earlier",
      preview: "Decision",
      responsePreview: "Use this approach",
    });
    expect(items[3]).toMatchObject({
      id: "highlight:later",
      preview: "later text",
    });
    expect(items[4]).toMatchObject({ kind: "turn", ordinal: 2 });
  });
});
