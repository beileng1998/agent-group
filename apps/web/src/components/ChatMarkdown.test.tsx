...
import { MessageId, ThreadMarkerId, type ThreadMarker } from "@agent-group/contracts";
...
async function renderMarkdown(
  text: string,
  cwd = "C:\\Users\\LENOVO\\agent-group",
...
describe("ChatMarkdown", () => {
  ...
  it("keeps dollar signs in markdown file links from becoming math", async () => {
    const source =
      "Files touched:\n\n- [_chat.$threadId.tsx](/Users/tester/project/apps/web/src/routes/_chat.$threadId.tsx:1192)";
    const markup = await renderMarkdown(source, "/Users/tester/project");

    expect(markup).toContain(
      'href="/Users/tester/project/apps/web/src/routes/_chat.$threadId.tsx:1192"',
    );
  ...
  it("preserves raw source offsets across multi-line Markdown blocks", async () => {
    const text = "- First saved line\n- Second saved line";
    const markup = await renderMarkdown(text);

    expect(markup).toContain(`data-transcript-source-start="${text.indexOf("First")}"`);
    expect(markup).toContain(`data-transcript-source-start="${text.indexOf("Second")}"`);
  });
  ...
  it("keeps plan, diff, and transcript surfaces routed through the shared renderer", () => {
    ...
    const assistantMessageRowSource = readFileSync(
      new URL("./chat/MessagesTimeline.assistantRow.tsx", import.meta.url),
      "utf8",
    );
    ...
    expect(assistantMessageRowSource).toContain('import ChatMarkdown from "../ChatMarkdown"');
    expect(assistantMessageRowSource).toContain("<ChatMarkdown");
  });
});
