...
const TERMINAL_PERF_STORAGE_KEY = "agent-group:terminal-perf";
...
function getTerminalPerfStore() {
  window.__agentGroupTerminalPerf ??= {
    samples: [],
    reset() {
      this.samples.length = 0;
    },
  };
  return window.__agentGroupTerminalPerf;
}
