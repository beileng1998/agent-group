...
import { ProjectId } from "@agent-group/contracts";
...
describe("EditorWorkspaceView", () => {
    ...
    const markup = renderToStaticMarkup(
      <QueryClientProvider client={queryClient}>
        <EditorWorkspaceView
          workspaceRoot={null}
          projectName="project"
          selectedFilePath="/tmp/agent-group-codex-workspaces/thread-1/report.pdf"
          expandedDirectories={new Set()}
          centerMode="file"
          diffFiles={[]}
          selectedDiffFilePath={null}
  ...
  it("renders scratch-workspace image previews without an attached workspace", () => {
    ...
    const markup = renderToStaticMarkup(
      <QueryClientProvider client={queryClient}>
        <EditorWorkspaceView
          workspaceRoot={null}
          projectName="project"
          selectedFilePath="/tmp/agent-group-codex-workspaces/thread-1/shot.png"
          expandedDirectories={new Set()}
          centerMode="file"
          diffFiles={[]}
          selectedDiffFilePath={null}
    ...
    expect(markup).toContain("local-image-preview");
    expect(markup).toContain(
      "/api/local-image?path=%2Ftmp%2Fagent-group-codex-workspaces%2Fthread-1%2Fshot.png",
    );
    expect(markup).not.toContain("No workspace is attached");
