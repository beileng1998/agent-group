...
describe("rankSettingsSearchEntries", () => {
  ...
  it("does not surface settings outside the Agent Group product model", () => {
    for (const query of [
      "worktree",
      "archived threads",
      "AppSnap",
      ...
      expect(rankSettingsSearchEntries(query, 12)).toHaveLength(0);
    }
  });
...
describe("SettingsSidebarNav", () => {
  it("keeps only settings with a current Agent Group consumer", () => {
    ...
    expect(normalizeSettingsSection("general")).toBe("appearance");
    expect(normalizeSettingsSection("worktrees")).toBe("appearance");
    expect(normalizeSettingsSection("models")).toBe("appearance");
    expect(normalizeSettingsSection("agent-group")).toBe("agent-group");
  });
  ...
  it("renders the soft search input alongside the section list", () => {
    ...
    expect(markup).not.toContain("Worktrees");
    expect(markup).not.toContain("AppSnap");
  });
});
