...
const SIDECHAT_THREAD_ID = "thread-browser-test-sidechat" as ThreadId;
...
const SIDECHAT_RESPONSE_TEXT = "Persisted Side reply loaded from thread detail.";
...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    keybindingsConfigPath: "/repo/project/.agent-group-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
...
function createSnapshotWithDockSidechat(): OrchestrationReadModel {
  const snapshot = createSnapshotForTargetUser({
    targetMessageId: MessageId.makeUnsafe("msg-user-sidechat-source"),
    targetText: "source",
  });
  const sourceThread = snapshot.threads[0];
  if (!sourceThread) {
    throw new Error("Missing source thread fixture.");
  }
  const sidechatThread = {
    ...sourceThread,
    id: SIDECHAT_THREAD_ID,
    title: "Sidechat: persisted response",
    parentThreadId: null,
    ...
    messages: [
      createUserMessage({
        id: MessageId.makeUnsafe("msg-user-sidechat"),
        text: "Explain the selection.",
        offsetSeconds: 200,
      }),
      createAssistantMessage({
        id: MessageId.makeUnsafe("msg-assistant-sidechat"),
        text: SIDECHAT_RESPONSE_TEXT,
        offsetSeconds: 201,
      }),
    ],
  ...
  return {
    ...snapshot,
    threads: [sourceThread, sidechatThread],
  };
}
  ...
  return {
    ...
    projects: [
      ...snapshot.projects,
      {
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/Users/tester/Documents/Agent Group/Studio",
        defaultModelSelection: {
          provider: "codex",
          model: "gpt-5",
        },
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("hydrates a dock sidechat through its own thread-detail subscription", async () => {
    useRightDockStore.getState().openPane(THREAD_ID, {
      paneId: "sidechat-browser-test",
      kind: "sidechat",
      threadId: SIDECHAT_THREAD_ID,
    });
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotWithDockSidechat(),
    });
          ...
          expect(
            wsRequests.some(
              (request) =>
                ...
                "threadId" in request &&
                request.threadId === SIDECHAT_THREAD_ID,
            ),
          ).toBe(true);
          expect(document.body.textContent).toContain(SIDECHAT_RESPONSE_TEXT);
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("keeps an Ask in Side selection in the composer until the user sends a question", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: MessageId.makeUnsafe("msg-assistant-sidechat-source"),
        targetText: "Selected explanation",
      }),
    });

    try {
      await vi.waitFor(() => {
        expect(getSidechatCreator(THREAD_ID)).toBeDefined();
      });
      wsRequests.length = 0;
      await getSidechatCreator(THREAD_ID)!({
        selection: {
          assistantMessageId: "msg-assistant-sidechat-source",
      ...
      expect(forkCommand).toBeTruthy();
      expect(forkCommand?.title).toBe(TEMPORARY_SIDECHAT_PLACEHOLDER_TITLE);
      const sidechatThreadId = forkCommand?.threadId as ThreadId;
      expect(
        wsRequests.some((request) => {
          const command = readDispatchedCommand(request);
          return command?.type === "thread.turn.start" && command.threadId === sidechatThreadId;
        }),
      ).toBe(false);
      expect(
        useComposerDraftStore.getState().draftsByThreadId[sidechatThreadId]?.assistantSelections,
      ).toMatchObject([
        {
          assistantMessageId: "msg-assistant-sidechat-source",
          text: "Selected explanation",
      ...
      expect(
        useRightDockStore
          .getState()
          .dockStateByThreadId[THREAD_ID]?.panes.some(
            (pane) => pane.kind === "sidechat" && pane.threadId === sidechatThreadId,
          ),
      ).toBe(true);
    } finally {
      await mounted.cleanup();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            cwd: "/repo/project",
            env: {
              AGENT_GROUP_PROJECT_ROOT: "/repo/project",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              AGENT_GROUP_PROJECT_ROOT: "/repo/project",
              AGENT_GROUP_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
    ...
    const mounted = await mountChatView({
      ...
      configureFixture: (nextFixture) => {
        nextFixture.welcome = {
          ...
          chatWorkspaceRoot: "/Users/tester/Documents/Agent Group",
          studioWorkspaceRoot: "/Users/tester/Documents/Agent Group/Studio",
        };
      },
    });
    ...
    try {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(createWorktreeRequest?.newBranch).toMatch(/^agent-group\/[0-9a-f]{8}$/);
  ...
  it("runs the setup action from the newly-created worktree before starting the turn", async () => {
    ...
    try {
      ...
      expect(terminalOpenRequest).toMatchObject({
        ...
        env: {
          AGENT_GROUP_PROJECT_ROOT: "/repo/project",
          AGENT_GROUP_WORKTREE_PATH: worktreePath,
        },
      });
