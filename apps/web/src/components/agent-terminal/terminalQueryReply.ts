// VENDORED from stablyai/orca @ 8a236183
// (src/shared/terminal-query-reply.ts) — MIT © 2026 Lovecast Inc.
// See apps/server/src/terminalHost/NOTICE.md.
//
// xterm's public onData stream mixes user input with synthetic replies to
// terminal queries. The server headless xterm owns those replies, so the view
// drops only complete known reply grammars to prevent duplicate PTY input.

const ESC = String.fromCharCode(0x1b);

/* oxlint-disable no-control-regex -- these grammars match terminal control sequences */
...
export function isTerminalQueryReply(data: string): boolean {
  if (data.length < 3 || data[0] !== ESC) return false;
  return (
