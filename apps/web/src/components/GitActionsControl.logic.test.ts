...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Agent Group as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "agent-group/fix-toast-copy");
  });

  it("normalizes an existing legacy agent-group namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "agent-group/refine-toolbar-actions");
    assert.equal(branch, "agent-group/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Agent Group", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "agent-group/feature/refine-toolbar-actions");
  });
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "agent-group/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "agent-group/deadbeef";
