...
// Purpose: Routes native macOS AppSnaps into the correct Agent Group composer draft.
// Layer: Root web coordinator
// Depends on: Desktop bridge, focused chat context, and existing composer attachment intake.

import { type DesktopAppSnapCapture, type ThreadId } from "@agent-group/contracts";
...
import { isComposerImageBlobReferenced, useComposerDraftStore } from "../composerDraftStore";
...
import {
  ...
  isThreadAvailable,
  rememberCaptureId,
  sourceWithCachedIcon,
  type PersistedAppSnapHydrationTarget,
} from "./app-snap/appSnapRuntimeValues";
import { useAppSnapTargetActivation } from "./app-snap/useAppSnapTargetActivation";
...
export function AppSnapCoordinator() {
  ...
  const activateExistingTarget = useAppSnapTargetActivation({
    focusedTargetRef,
    openChatThreadPage,
  });
    ...
    async (capture: DesktopAppSnapCapture) => {
      ...
      if (resolvedTarget.kind === "existing") {
        ...
        if (result.threadId) {
          ...
          if (!focused) throw new Error("Agent Group could not create a task for this AppSnap.");
          target = focused;
          openChatThreadPage(target.threadId);
        }
      }
      ...
      if (!image) throw new Error(error ?? "Agent Group could not attach the captured AppSnap.");
      ...
      toastManager.add({
        ...
        description:
          persistenceResult === "unverified"
            ? "The capture is attached, but Agent Group could not verify its draft metadata. If it is missing after a reload, Agent Group will attach it again."
