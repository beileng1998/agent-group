...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/tester/.agent-group/worktrees/agent-group-mvp/agent-group-4e609bb8",
    );
    expect(result).toBe("agent-group-4e609bb8");
  });

  it("keeps legacy .agent-group worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/tester/.agent-group/worktrees/agent-group-mvp/agent-group-legacy123",
    );
    expect(result).toBe("agent-group-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.agent-group\\worktrees\\agent-group-mvp\\agent-group-4e609bb8",
    );
    expect(result).toBe("agent-group-4e609bb8");
  });

  it("uses the final segment even when outside ~/.agent-group/worktrees", () => {
