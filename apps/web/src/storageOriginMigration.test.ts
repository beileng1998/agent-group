...
describe("storageOriginMigration", () => {
  ...
  it("imports missing keys without overwriting current-origin state", async () => {
    globalThis.localStorage.setItem("agent-group:theme", "current");
    const { importAgentGroupStorageSnapshot } = await import("./storageOriginMigration");

    expect(
      importAgentGroupStorageSnapshot({
        version: 1,
        exportedAt: "2026-07-09T00:00:00.000Z",
        entries: {
          "agent-group:theme": "snapshot",
          "agent-group:composer-drafts:v1": "draft",
        },
      }),
    ).toBe(true);
    expect(globalThis.localStorage.getItem("agent-group:theme")).toBe("current");
    expect(globalThis.localStorage.getItem("agent-group:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("rejects an invalid snapshot before writing any entry", async () => {
    ...
    expect(globalThis.localStorage.getItem("agent-group:theme")).toBeNull();
  });
  ...
  it("imports snapshots containing large composer drafts", async () => {
    ...
    expect(globalThis.localStorage.getItem("agent-group:composer-drafts:v1")).toBe(largeDraft);
  });
  ...
  it("keeps the snapshot retryable after a partial storage failure", async () => {
    ...
    const snapshot = {
      version: 1 as const,
      exportedAt: "2026-07-09T00:00:00.000Z",
      entries: { "agent-group:theme": "dark", "agent-group:composer-drafts:v1": "draft" },
    };

    expect(importAgentGroupStorageSnapshot(snapshot, storage)).toBe(false);
    storage.setItem = setItem;
    expect(importAgentGroupStorageSnapshot(snapshot, storage)).toBe(true);
    expect(storage.getItem("agent-group:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("acknowledges the desktop snapshot only after a complete bootstrap import", async () => {
      ...
      desktopBridge: {
        storageMigration: {
          readSnapshot: () => ({
            version: 1,
            exportedAt: "2026-07-09T00:00:00.000Z",
            entries: { "agent-group:theme": "dark" },
          }),
          acknowledgeSnapshot,
        },
      },
    ...
    expect(globalThis.localStorage.getItem("agent-group:theme")).toBe("dark");
  });
