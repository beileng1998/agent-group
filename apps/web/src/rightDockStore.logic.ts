...
import { AGENT_GROUP_PERSISTED_DOCK_KINDS } from "./agentGroupCapabilities";
import { isPlainObject, sanitizeStringKeyedRecord } from "./persistedRecord";
import { RIGHT_DOCK_PANE_KINDS, type RightDockPaneKind } from "./rightDockStore.contracts";
...
export { RIGHT_DOCK_PANE_KINDS };
export type { RightDockPaneKind };
...
const AGENT_GROUP_CONTROL_PANE_ORDER: Partial<Record<RightDockPaneKind, number>> = {
  context: 0,
  highlights: 1,
  group: 2,
};
...
function sanitizePersistedPane(value: unknown): RightDockPane | null {
  ...
  if (!AGENT_GROUP_PERSISTED_DOCK_KINDS.has(candidate.kind)) {
    return null;
  }
  return {
    id: candidate.id,
...
function orderAgentGroupControlPanes(panes: RightDockPane[]): RightDockPane[] {
  return panes.toSorted(
    (left, right) =>
      (AGENT_GROUP_CONTROL_PANE_ORDER[left.kind] ?? Number.MAX_SAFE_INTEGER) -
      (AGENT_GROUP_CONTROL_PANE_ORDER[right.kind] ?? Number.MAX_SAFE_INTEGER),
  );
}
...
export function sanitizeRightDockThreadState(value: unknown): RightDockThreadState {
  ...
  const panes = Array.isArray(candidate.panes)
    ...
    : [];
  const orderedPanes = orderAgentGroupControlPanes(panes);
  const activePaneId =
    typeof candidate.activePaneId === "string" &&
    orderedPanes.some((pane) => pane.id === candidate.activePaneId)
      ? candidate.activePaneId
      : (orderedPanes[0]?.id ?? null);
  return {
    open: orderedPanes.length > 0 && candidate.open === true,
    panes: orderedPanes,
    activePaneId,
  };
}
...
export function openPaneInState(
  ...
  return {
    open: true,
    panes: orderAgentGroupControlPanes([...state.panes, pane]),
    activePaneId: pane.id,
  };
}

// Adds a pane without changing whether the dock is visible. This is used for
// control panes that must exist for a thread but should not force the dock open.
export function ensurePaneInState(
  state: RightDockThreadState,
  input: OpenPaneInput,
): RightDockThreadState {
  const next = openPaneInState(state, input);
  return {
    ...next,
    open: state.open,
    activePaneId: state.activePaneId ?? next.activePaneId,
  };
}
...
export function closePaneInState(
  ...
  if (state.panes[removedIndex]?.kind === "context") {
    return state;
  }
  if (removedIndex === -1) {
    return state;
