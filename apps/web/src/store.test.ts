...
describe("store pure functions", () => {
  it("uses the completed turn timestamp as an idempotent visited watermark", () => {
    ...
    const visited = markThreadVisited(initialState, threadId, completedAt);
    const repeated = markThreadVisited(visited, threadId, completedAt);

    expect(visited.threads[0]?.lastVisitedAt).toBe(completedAt);
    expect(repeated).toBe(visited);
  });
  ...
  it("preserves optimistic createBranchFlowCompleted during stale read-model syncs", () => {
    ...
    const optimisticState = setThreadWorkspace(
      makeState(
        makeThread({
          envMode: "worktree",
          branch: "agent-group/tmp-working",
          worktreePath: "/tmp/project/.worktrees/tmp-working",
          associatedWorktreePath: "/tmp/project/.worktrees/tmp-working",
          associatedWorktreeBranch: "agent-group/tmp-working",
          associatedWorktreeRef: "agent-group/tmp-working",
        }),
      ),
      threadId,
      {
  ...
  it("applies sidechat promotion metadata immediately during live updates", () => {
    const sourceThreadId = ThreadId.makeUnsafe("thread-main");
    const initialState = makeState(
      makeThread({
        parentThreadId: null,
  ...
  it("renames a project locally without changing its remote or folder names", () => {
    ...
    const next = renameProjectLocally(state, ProjectId.makeUnsafe("project-1"), "agent-group");

    expect(next.projects[0]).toMatchObject({
      name: "agent-group",
      localName: "agent-group",
      remoteName: "Project",
      folderName: "project",
    });
  });
...
describe("store read model sync", () => {
  it("adds the desktop bridge token to server attachment preview URLs", () => {
    ...
    const testWindow = {
      location: { origin: "agent-group://app" },
      desktopBridge: {
  ...
  it("keeps a cleared local project alias from reappearing during syncs", async () => {
    ...
    storage.set(
      "agent-group:renderer-state:v8",
      JSON.stringify({
        projectNamesByCwd: {
          "/tmp/project": "agent-group",
        },
      }),
    );
  ...
  it("persists project aliases immediately when the local alias changes", async () => {
    ...
    try {
      ...
      freshStore.useStore.getState().renameProjectLocally(projectId, "agent-group");

      expect(setItem).toHaveBeenCalled();
      expect(JSON.parse(storage.get("agent-group:renderer-state:v8") ?? "{}")).toMatchObject({
        projectNamesByCwd: {
          "/tmp/project": "agent-group",
        },
      });
