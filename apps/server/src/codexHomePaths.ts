...
export const AGENT_GROUP_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveAgentGroupCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.AGENT_GROUP_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".agent-group", "runtime");
  return path.join(overlayRoot, AGENT_GROUP_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Agent Group keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveAgentGroupCodexHomeOverlayPath(env, source);
