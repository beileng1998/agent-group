...
async function makeConfig(overrides: Partial<ServerConfigShape> = {}): Promise<ServerConfigShape> {
  const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-http-test-"));
...
async function makeHandler(
  ...
  auth?: {
    ...
    readonly renew?: SessionCredentialServiceShape["renew"];
  },
    ...
    ...(auth
      ? {
          ...
          sessionCredentials: {
            ...
            renew:
              auth.renew ?? (() => Effect.die("Session renewal was not expected in this test.")),
          },
        }
      : {}),
  });
...
function makeAuthDescriptor() {
  return {
    ...
    sessionCookieName: "agent_group_session",
  };
}
...
describe("createHttpRequestHandler", () => {
  ...
  it("serves static files and SPA fallback", async () => {
    const staticDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-static-test-"));
  ...
  it("serves cached native editor icons before dev/static fallback", async () => {
    const homeDir = makeTempDir("agent-group-http-editor-icon-home-");
  ...
  it("sets a session cookie on auth bootstrap", async () => {
    ...
    await withServer(handler, async (origin) => {
      ...
      expect(response.status).toBe(200);
      expect(response.headers.get("set-cookie")).toContain("agent_group_session=session-token");
      await expect(response.json()).resolves.toMatchObject({
        authenticated: true,
        sessionMethod: "browser-session-cookie",
      });
  ...
  it("renews an active browser session before it expires", async () => {
    const config = await makeConfig();
    const now = Effect.runSync(DateTime.now);
    const currentExpiresAt = DateTime.addDuration(now, Duration.days(1));
    const renewedExpiresAt = DateTime.addDuration(now, Duration.days(30));
    const handler = await makeHandler(config, {
      serverAuth: makeFakeServerAuth({
        getSessionState: () =>
          Effect.succeed({
            ...
            role: "client",
            sessionMethod: "browser-session-cookie",
            expiresAt: DateTime.toUtc(currentExpiresAt),
          }),
      }),
      cookieName: "agent_group_session",
      renew: () =>
    ...
    await withServer(handler, async (origin) => {
      const response = await fetch(`${origin}/api/auth/session`, {
        headers: { Cookie: "agent_group_session=old-session-token" },
      });

      expect(response.status).toBe(200);
      expect(response.headers.get("set-cookie")).toContain("agent_group_session=renewed-session-token");
      await expect(response.json()).resolves.toMatchObject({
        authenticated: true,
        expiresAt: DateTime.formatIso(renewedExpiresAt),
      });
    });
  });
});
