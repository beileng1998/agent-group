...
//          that makes Agent Group catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@agent-group/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    ...
    case "codex":
      ...
      // folders plus `~/.agent-group/skills`, which Agent Group registers at session start
    ...
    case "cursor":
      ...
      // only Agent Group-owned paths need inlining.
      return segments.has(".agent-group");
    case "claudeAgent":
