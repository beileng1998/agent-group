import type { ProviderEvent } from "@agent-group/contracts";

export const MAX_UNMAPPED_PROVIDER_DATA_JSON_CHARS = 16_000;

const MAX_DETAIL_CHARS = 500;
const MAX_NATIVE_TYPE_CHARS = 200;
...
export function sanitizeUnmappedProviderData(value: unknown): unknown {
  ...
  if (serialized.length <= MAX_UNMAPPED_PROVIDER_DATA_JSON_CHARS) return redacted;
  return {
    __agentGroupTruncated: true,
    originalJsonChars: serialized.length,
    preview: `${serialized.slice(0, MAX_PREVIEW_CHARS - 3)}...`,
  };
}

export function sanitizeUnmappedProviderDetail(value: string | undefined): string | undefined {
  return value === undefined ? undefined : sanitizeText(value, MAX_DETAIL_CHARS);
}

export function sanitizeUnmappedProviderNativeType(value: string): string {
  return sanitizeText(value, MAX_NATIVE_TYPE_CHARS);
}
...
export function makeUnmappedProviderEventGate(maxTrackedBursts = 128) {
  ...
  return (event: ProviderEvent): boolean => {
    ...
    const scope = event.turnId ?? event.providerThreadId ?? "session";
    const key = `${event.provider}\u0000${event.threadId}\u0000${scope}\u0000${sanitizeUnmappedProviderNativeType(event.method)}`;
