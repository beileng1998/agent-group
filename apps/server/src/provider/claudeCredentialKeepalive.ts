...
//
// Opt in:   AGENT_GROUP_CLAUDE_KEEPALIVE=1
// Tune:     AGENT_GROUP_CLAUDE_KEEPALIVE_MINUTES=<n>   (default 30)
...
export function isClaudeCredentialKeepaliveEnabled(
  ...
  return platform === "darwin" && envFlagEnabled(env.AGENT_GROUP_CLAUDE_KEEPALIVE);
}
...
export function resolveClaudeCredentialKeepaliveIntervalMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AGENT_GROUP_CLAUDE_KEEPALIVE_MINUTES?.trim();
...
export function startClaudeCredentialKeepalive(input?: {
  ...
  // Agent Group should not do it as background work merely because the app opened.
