...
describe("toOpenCodeFileParts", () => {
  it("materializes image attachments as SDK file parts", () => {
    ...
    expect(
      toOpenCodeFileParts({
        ...
        resolveAttachmentPath: () => "/tmp/agent-group-attachments/screenshot.png",
      }),
    ).toEqual([
      {
        type: "file",
        ...
        url: "file:///tmp/agent-group-attachments/screenshot.png",
      },
    ]);
  });
  ...
  it("leaves generic files for prompt-path projection", () => {
    ...
    expect(
      toOpenCodeFileParts({
        attachments: [attachment],
        resolveAttachmentPath: () => "/tmp/agent-group-attachments/notes.docx",
      }),
    ).toEqual([]);
  });
});
