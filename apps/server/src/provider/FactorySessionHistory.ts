...
async function findFactorySessionPathInDirectory(
  sessionsDir: string,
  sessionId: string,
): Promise<string | null> {
...
export async function findFactorySessionPath(
  homeDir: string,
  sessionId: string,
): Promise<string | null> {
  return findFactorySessionPathInDirectory(
...
export async function readFactorySessionHistory(
  ...
  const path = await findFactorySessionPath(homeDir, normalizedSessionId);
  if (!path) return null;
