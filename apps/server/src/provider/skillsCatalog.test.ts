...
describe("discoverSkillsCatalog", () => {
  ...
  it("aggregates skills from agent-group and provider home folders with origin scopes", async () => {
    await writeSkill(
      path.join(agentGroupBaseDir, "skills", "portable"),
      "portable",
      "Agent Group skill",
    );
  ...
  it("prefers the provider-native copy and falls back to Agent Group for that provider", async () => {
    await writeSkill(
      path.join(agentGroupBaseDir, "skills", "shared"),
      "shared",
      "Agent Group copy",
    );
    await writeSkill(path.join(homeDir, ".codex", "skills", "shared"), "shared", "Codex copy");
    await writeSkill(
      path.join(agentGroupBaseDir, "skills", "only-agent-group"),
      "only-agent-group",
      "Fallback",
    );

    const codexView = await discoverSkillsCatalog({
      homeDir,
      agentGroupBaseDir,
      provider: "codex",
    });
