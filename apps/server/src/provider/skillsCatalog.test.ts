...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "agent-group-skills-catalog-"));
...
describe("discoverSkillsCatalog", () => {
  it("creates the Agent Group skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, agentGroupBaseDir });
    await expect(access(path.join(agentGroupBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from agent-group and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(agentGroupBaseDir, "skills", "portable"), "portable", "Agent Group skill");
    ...
    expect(byName.get("portable")?.scope).toBe("agent-group");
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, agentGroupBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Agent Group for that provider", async () => {
    await writeSkill(path.join(agentGroupBaseDir, "skills", "shared"), "shared", "Agent Group copy");
    await writeSkill(path.join(homeDir, ".codex", "skills", "shared"), "shared", "Codex copy");
    await writeSkill(path.join(agentGroupBaseDir, "skills", "only-agent-group"), "only-agent-group", "Fallback");

    const codexView = await discoverSkillsCatalog({ homeDir, agentGroupBaseDir, provider: "codex" });
    ...
    expect(codexView.some((skill) => skill.name === "only-agent-group")).toBe(true);

    // A provider without its own copy resolves the Agent Group fallback.
    ...
    expect(claudeShared?.scope).toBe("agent-group");
  });

  it("uses documented provider alias roots before Agent Group fallbacks", async () => {
    await writeSkill(path.join(agentGroupBaseDir, "skills", "shared"), "shared", "Agent Group copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(agentGroupBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, agentGroupBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(agentGroupBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, agentGroupBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, agentGroupBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .agent-group skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".agent-group", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(agentGroupBaseDir, "skills", "portable"), "portable", "Agent Group");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("agent-group");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(agentGroupBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(agentGroupBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "agent-group"), descriptor("extra", "agent-group")],
    });
