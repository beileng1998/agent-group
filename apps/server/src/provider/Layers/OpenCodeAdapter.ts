    ...
    Effect.gen(function* () {
        ...
        if (!context.activeTurnToolCallIdleWatchdogStarted) {
          ...
          yield* Effect.gen(function* () {
            ...
            yield* completeOpenCodeTurn(context, {
              turnId,
              raw: {
                source: "agent-group.opencode.idle-after-tool-calls",
                event: raw,
              },
      ...
      const recoverOpenCodeTurnFromMessages = Effect.fn("recoverOpenCodeTurnFromMessages")(
        function* (
          ...
          return yield* recoverOpenCodeTurnFromAssistantMessage(context, {
            ...
            assistantEntry,
            raw: {
              source: "agent-group.opencode.prompt.recovery",
        ...
        yield* Effect.gen(function* () {
          ...
          Effect.flatMap(() =>
            Effect.gen(function* () {
              ...
              yield* completeOpenCodeTurn(context, {
                turnId: input.turnId,
                raw: { source: "agent-group.opencode.prompt.watchdog" },
                errorMessage: message,
              });
              updateProviderSession(
                context,
            ...
            Effect.gen(function* () {
              ...
              if (assistantEntry && isOpenCodeCompletedAssistantMessage(assistantEntry)) {
                yield* recoverOpenCodeTurnFromAssistantMessage(context, {
                  ...
                  assistantEntry,
                  raw: {
                    source: "agent-group.opencode.prompt.response",
                    message: assistantEntry,
                  },
                });
              }
      ...
      const handleSubscribedEvent = Effect.fn("handleSubscribedEvent")(function* (
        ...
        switch (event.type) {
          ...
          // agent loop. Mirror them into Agent Group's canonical transcript stream.
                ...
                const createSessionId = resumedSessionId
                  ...
                  : runOpenCodeSdk("session.create", () => {
                      const sessionCreateInput = {
                        ...
                        permission: buildOpenCodePermissionRules(input.runtimeMode),
                        title: `Agent Group ${input.threadId}`,
                      };
      ...
      const sendTurn: OpenCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
        ...
        // Always pin Agent Group's interaction mode to OpenCode's primary agent.
        ...
        return Effect.gen(function* () {
          ...
          if (Exit.isSuccess(inventoryExit)) {
            ...
            const models = mergeOpenCodeCliModelDescriptors({
              ...
              // `opencode models` is already scoped to usable providers. Preserve
              // every CLI row even when inventory ranking prefers a managed provider;
              // configured providers can be connected without an auth.json entry.
              cliModels,
              ...(freeOnlyProviderID ? { freeOnlyProviderID } : {}),
            });
