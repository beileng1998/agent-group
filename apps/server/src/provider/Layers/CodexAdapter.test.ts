...
class FakeCodexManager extends CodexAppServerManager {
  ...
  public stopAllImpl = vi.fn(async () => undefined);
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("keeps imageView as inspection activity instead of generated output", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      assert.equal(firstEvent.value.payload.detail, "/tmp/inspection.png");
    }),
  );
  ...
  it.effect("does not project retryable Codex error notifications", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      lifecycleManager.emit("event", {
        id: asEventId("evt-after-retryable-error"),
        kind: "session",
        provider: "codex",
        threadId: asThreadId("thread-1"),
        createdAt: new Date().toISOString(),
      ...
      assert.equal(firstEvent.value.eventId, "evt-after-retryable-error");
      assert.equal(firstEvent.value.type, "session.exited");
    }),
  );
  ...
  it.effect("does not project process stderr provider errors", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      lifecycleManager.emit("event", {
        id: asEventId("evt-after-process-stderr"),
        kind: "session",
        provider: "codex",
        threadId: asThreadId("thread-1"),
        createdAt: new Date().toISOString(),
      ...
      assert.equal(firstEvent.value.eventId, "evt-after-process-stderr");
      assert.equal(firstEvent.value.type, "session.exited");
    }),
  );
