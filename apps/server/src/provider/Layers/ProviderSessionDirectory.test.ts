...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))("ProviderSessionDirectoryLive", (it) => {
  ...
  it("resets provider-specific runtime state when provider changes", () =>
    Effect.gen(function* () {
      const directory = yield* ProviderSessionDirectory;
      const runtimeRepository = yield* ProviderSessionRuntimeRepository;
      ...
      yield* runtimeRepository.upsert({
        ...
        resumeCursor: { threadId: "claude-session" },
        runtimePayload: {
          cwd: "/tmp/old",
      ...
      if (Option.isSome(runtime)) {
        ...
        assert.deepEqual(runtime.value.runtimePayload, { cwd: "/tmp/new" });
      }
    }));

  it("rehydrates persisted mappings across layer restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-provider-directory-"));
  ...
  it("rehydrates persisted OpenCode bindings across layer restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-provider-directory-opencode-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const directoryLayer = makeDirectoryLayer(makeSqlitePersistenceLive(dbPath));
