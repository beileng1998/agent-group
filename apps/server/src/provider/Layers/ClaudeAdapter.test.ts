...
import {
  FakeClaudeQuery,
  makeClaudeAdapterTestHarness as makeHarness,
  makeMultiQueryClaudeAdapterTestHarness as makeMultiQueryHarness,
} from "./ClaudeAdapter.testHarness.ts";
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("discovers resolved custom Claude models without an active session", () => {
    const harness = makeHarness();
    harness.query.setSupportedModels([
      {
        value: "default",
        resolvedModel: "kimi-k3[1m]",
        displayName: "Default (recommended)",
        description: "Use the configured default model",
      },
      {
        value: "opus",
      ...
      {
        value: "haiku",
        resolvedModel: "doubao-seed-evolving",
    ...
    return Effect.gen(function* () {
      ...
      const input = {
        ...
        cwd: "/repo/custom-claude",
      };
      const result = yield* listModels(input);
      ...
      assert.equal(harness.getLastCreateQueryInput()?.options.cwd, "/repo/custom-claude");
      assert.equal(
      ...
      const cached = yield* listModels(input);
  ...
  it.effect("passes an explicitly configured Claude turn limit to the SDK", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
      ...
      assert.equal(harness.getLastCreateQueryInput()?.options.maxTurns, 96);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("classifies Claude Task tool invocations as collaboration agent work", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (toolStarted?.type === "item.started") {
        ...
        assert.equal(data.prompt, "Audit the SQL changes");
      }
