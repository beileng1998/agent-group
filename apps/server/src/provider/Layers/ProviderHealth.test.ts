...
import { DEFAULT_SERVER_SETTINGS, ServerProviderUpdateError } from "@agent-group/contracts";
...
function withTempCodexHome(configContent?: string) {
  return Effect.gen(function* () {
    ...
    const tmpDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "agent-group-test-codex-" });
    const runtimeDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "agent-group-test-runtime-",
    });
  ...
  describe("disabled provider handling", () => {
    it("builds an inert status for disabled providers", () => {
      assert.deepStrictEqual(makeDisabledProviderStatus("kilo", "2026-06-16T12:00:00.000Z"), {
        ...
        authStatus: "unknown",
        checkedAt: "2026-06-16T12:00:00.000Z",
        message: "Provider is disabled in Agent Group settings.",
      });
    });
    ...
    it("projects disabled settings over cached ready statuses", () => {
      ...
      assert.strictEqual(codex?.message, "Provider is disabled in Agent Group settings.");
    });
    ...
    it.effect("projects cached ready status when a disabled provider is re-enabled", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(
            disabledCodex?.message,
            "Provider is disabled in Agent Group settings.",
          );
    ...
    it.effect("does not offer updates for disabled providers", () =>
      Effect.gen(function* () {
        ...
        for (const status of statuses) {
          ...
          assert.strictEqual(status.message, "Provider is disabled in Agent Group settings.");
    ...
    it.effect("rejects one-click updates for disabled providers", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(error.reason, "Provider is disabled in Agent Group settings.");
      }).pipe(Effect.provide(disabledProviderHealthLayer)),
    );
  });
    ...
    it.effect("uses configured codex home for version, config, and auth probes", () => {
      ...
      return Effect.gen(function* () {
        ...
        const configuredHome = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "agent-group-configured-codex-",
        });
        yield* fileSystem.writeFileString(
          path.join(configuredHome, "config.toml"),
          'model_provider = "openai"\n',
        );
        expectedCodexHome = path.join(runtimeDir, AGENT_GROUP_CODEX_HOME_OVERLAY_DIR);
  ...
  describe("checkPiProviderStatus", () => {
    ...
    it.effect("uses configured Pi binary and agent dir without SDK registry reads", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Pi CLI is installed. Agent Group will use Pi agent dir /tmp/pi-agent.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args, command) => {
  ...
  describe("checkAntigravityProviderStatus", () => {
    it.effect("rejects versions that predate --new-project support", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Antigravity CLI 1.0.11 is too old for Agent Group. Upgrade to 1.0.12 or newer.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args) => {
