...
describe("OpenCodeAdapter runtime lifecycle", () => {
  ...
  it("keeps connected config-provider models alongside preferred OpenCode models", async () => {
    const runtime = createMockOpenCodeRuntime({
      cliModels: [
        {
      ...
      inventory: {
        providerList: {
          connected: ["opencode", "volcengine-ark"],
          default: {},
          all: [
            makeProvider({
              id: "opencode",
  ...
  it("pins the initial model on new OpenCode sessions", async () => {
    ...
    expect(runtime.createCalls[0]).toMatchObject({
      ...
      agent: "build",
      title: "Agent Group thread-model-pin",
    });
  });
