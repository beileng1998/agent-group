...
import {
  ...
  type ReleaseListSessions,
  makeFakeCodexAdapter,
  makeProviderServiceLayer,
  requireReleaseListSessions,
  sleep,
  waitUntil,
  waitUntilEffect,
...
it.effect("ProviderServiceLive keeps persisted resumable sessions on startup", () =>
  Effect.gen(function* () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-provider-service-"));
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-provider-service-stopall-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-provider-service-restart-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
...
routing.layer("ProviderServiceLive routing", (it) => {
  it.effect("resolves only existing absolute provider transcript files", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService;
      const directory = fs.mkdtempSync(path.join(os.tmpdir(), "provider-transcript-path-"));
      const transcriptPath = path.join(directory, "session.jsonl");
      fs.writeFileSync(transcriptPath, "{}\n", "utf8");
      const threadId = asThreadId("thread-transcript-path");

      yield* provider.startSession(threadId, {
        provider: "codex",
        threadId,
        cwd: "/tmp/project",
        resumeCursor: { transcriptPath },
        runtimeMode: "full-access",
      });
      const resolveTranscriptPath = provider.resolveTranscriptPath;
      if (!resolveTranscriptPath) {
        assert.fail("Expected ProviderService.resolveTranscriptPath");
      }
      assert.equal(yield* resolveTranscriptPath({ threadId }), transcriptPath);

      fs.rmSync(transcriptPath);
      assert.equal(yield* resolveTranscriptPath({ threadId }), null);
  ...
  it.effect("reuses persisted resume cursor when startSession is called after a restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-provider-service-start-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
  ...
  it.effect("clears stale resume cursor while preserving provider options for fresh restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-provider-service-clear-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
  ...
  it.effect("stops the live runtime while preserving resume cursor and provider options", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "agent-group-provider-service-stop-runtime-"),
      );
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
...
fanout.layer("ProviderServiceLive fanout", (it) => {
  ...
  it.effect("ignores delayed events from a provider that no longer owns the binding", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService;
      const directory = yield* ProviderSessionDirectory;
      const threadId = asThreadId("thread-provider-replaced");
      ...
      fanout.codex.emit({
        type: "session.exited",
        eventId: asEventId("evt-old-codex-session-exited"),
        provider: "codex",
        createdAt: new Date().toISOString(),
        threadId,
