...
const make = Effect.gen(function* () {
  ...
  const getComposerCapabilities: ProviderDiscoveryServiceShape["getComposerCapabilities"] = (
    ...
    Effect.gen(function* () {
      ...
      // The unified Agent Group skills catalog backs skill discovery for every
    ...
    Effect.gen(function* () {
      ...
      const nativeResult: ProviderListSkillsResult | null = adapter.listSkills
        ? yield* adapter
            ...
            .pipe(
              Effect.catch((error) =>
                Effect.logWarning(
                  "provider-native skill discovery failed; serving the Agent Group skills catalog only",
      ...
      const catalogSkills = yield* Effect.tryPromise(() =>
        discoverSkillsCatalog({
          ...
          agentGroupBaseDir: serverConfig.baseDir,
          provider: parsed.provider,
          ...(parsed.forceReload !== undefined ? { forceReload: parsed.forceReload } : {}),
        }),
      ).pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("agent-group skills catalog discovery failed", {
            provider: parsed.provider,
            cause,
      ...
      return {
        ...
        source: nativeResult?.source
          ? `${nativeResult.source}+agent-group.catalog`
          : "agent-group.catalog",
