import { ProviderKind, type ThreadId } from "@agent-group/contracts";
  ...
  const upsert: ProviderSessionDirectoryShape["upsert"] = Effect.fn(function* (binding) {
    ...
    yield* repository
      .upsert({
        ...
        runtimeMode:
          binding.runtimeMode ??
          (providerChanged ? "full-access" : existingRuntime?.runtimeMode) ??
          "full-access",
        status:
          binding.status ?? (providerChanged ? "running" : existingRuntime?.status) ?? "running",
        lastSeenAt: now,
        resumeCursor:
