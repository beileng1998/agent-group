...
process.stdin.on("end", () => {
  const target = process.env.AGENT_GROUP_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.AGENT_GROUP_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    ...
    `${JSON.stringify(
      {
        ...
        name: "agent-group-capture",
        description: "Streams Antigravity CLI lifecycle events to Agent Group when requested.",
      },
      null,
      2,
    )}\n`,
...
const makeAntigravityAdapter = Effect.gen(function* () {
      ...
      yield* Effect.tryPromise({
        try: () => ensureCapturePlugin(binaryPath),
          ...
          new ProviderAdapterRequestError({
            ...
            detail: messageFromCause(cause, "Failed to install the Agent Group capture hook."),
            cause,
          }),
      });
  ...
  const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
    Effect.gen(function* () {
      ...
      const runDir = yield* Effect.tryPromise({
        try: () => fs.mkdtemp(path.join(os.tmpdir(), "agent-group-antigravity-")),
        catch: (cause) =>
          new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
      ...
      const child = spawn(context.binaryPath, args, {
        ...
        env: {
          ...process.env,
          AGENT_GROUP_ANTIGRAVITY_EVENTS: eventFile,
          AGENT_GROUP_ANTIGRAVITY_HOOK_DECISION: "allow",
        },
        stdio: ["ignore", "pipe", "pipe"],
      });
  ...
  const resolveTranscriptPath: NonNullable<AntigravityAdapterShape["resolveTranscriptPath"]> = (
    input,
  ) => {
    const conversationId = resumeConversationId(input.resumeCursor);
    return Effect.succeed(conversationId ? transcriptPathForConversation(conversationId) : null);
  };
