...
describe("Antigravity CLI integration helpers", () => {
  it("keeps the globally installed hook neutral outside Agent Group sessions", async () => {
  ...
  it("advances file offsets only past complete JSONL records", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "agent-group-antigravity-test-"));
