...
export interface SupervisedProcessTeardownInput {
  ...
  /** Resolves only after the owned root process emits its terminal exit. */
...
export class ProviderProcessExitUnprovenError extends Error {
  ...
  readonly retry?: (() => Promise<SupervisedProcessTeardownResult>) | undefined;
  ...
  constructor(input: {
    ...
    readonly captureComplete: boolean;
    readonly safeToRetry?: boolean;
    readonly retry?: () => Promise<SupervisedProcessTeardownResult>;
  }) {
    const descendants =
    ...
    super(
      `Provider process tree ${input.rootPid} did not prove exit ` +
        `(rootExited=${String(input.rootExited)}, captureComplete=${String(input.captureComplete)}; ${descendants}).`,
    );
    ...
    this.safeToRetry = input.safeToRetry === true;
...
/**
 * Stops one owned provider process tree. Success means the exact root emitted exit and every
 * identity-matched descendant captured before TERM is gone; sending a signal is not completion.
 */
export async function teardownProviderProcessTree(
  ...
  if (!Number.isInteger(input.rootPid) || input.rootPid <= 0) {
    throw new TypeError(`Provider process root PID must be positive, got ${input.rootPid}.`);
  }
  ...
  // Flush an already-settled exit handle before capturing or signalling a potentially reused PID.
  ...
  if (tree.captureComplete === false) {
    throw new ProviderProcessExitUnprovenError({
      rootPid: input.rootPid,
      rootExited: false,
      remainingDescendantPids: tree.descendants.map((descendant) => descendant.pid),
      captureComplete: false,
      safeToRetry: true,
    });
  }
  ...
  // Reuse this exact pre-TERM capture when an inspection fails after signalling. Recapturing could
  // lose reparented descendants or target unrelated processes that later reuse their PIDs.
  const forceUntilProven = async (): Promise<SupervisedProcessTeardownResult> => {
    ...
    throw new ProviderProcessExitUnprovenError({
      ...
      safeToRetry: true,
      retry: forceUntilProven,
    });
  };

  return forceUntilProven();
}
