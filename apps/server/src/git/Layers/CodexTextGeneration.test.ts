...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "agent-group-codex-text-generation-test-",
    }),
  ),
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "done",
        'stdin_content="$(cat)"',
        'if [ "$AGENT_GROUP_FAKE_CODEX_REQUIRE_IMAGE" = "1" ] && [ "$seen_image" != "1" ]; then',
        '  printf "%s\\n" "missing --image input" >&2',
        "  exit 2",
        "fi",
        'if [ "$AGENT_GROUP_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK" = "1" ] && [ "$seen_skip_git_repo_check" != "1" ]; then',
        '  printf "%s\\n" "missing --skip-git-repo-check" >&2',
        "  exit 9",
        "fi",
        'if [ "$AGENT_GROUP_FAKE_CODEX_REQUIRE_APPROVAL_NEVER" = "1" ] && [ "$seen_approval_never" != "1" ]; then',
        '  printf "%s\\n" "missing approval_policy=never" >&2',
        "  exit 10",
        "fi",
        'if [ -n "$AGENT_GROUP_FAKE_CODEX_STDIN_MUST_CONTAIN" ]; then',
        '  printf "%s" "$stdin_content" | grep -F -- "$AGENT_GROUP_FAKE_CODEX_STDIN_MUST_CONTAIN" >/dev/null || {',
        '    printf "%s\\n" "stdin missing expected content" >&2',
        "    exit 3",
        "  }",
        "fi",
        'if [ -n "$AGENT_GROUP_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" ]; then',
        '  if printf "%s" "$stdin_content" | grep -F -- "$AGENT_GROUP_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" >/dev/null; then',
        '    printf "%s\\n" "stdin contained forbidden content" >&2',
        "    exit 4",
        "  fi",
        "fi",
        'if [ "$AGENT_GROUP_FAKE_CODEX_REQUIRE_CODEX_HOME" = "1" ] && [ -z "$CODEX_HOME" ]; then',
        '  printf "%s\\n" "missing CODEX_HOME" >&2',
        "  exit 5",
        "fi",
        'if [ "$AGENT_GROUP_FAKE_CODEX_REQUIRE_AUTH_JSON" = "1" ] && [ ! -f "$CODEX_HOME/auth.json" ]; then',
        '  printf "%s\\n" "missing auth.json in CODEX_HOME" >&2',
        "  exit 6",
        "fi",
        'if [ -n "$AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN" ]; then',
        '  grep -F -- "$AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN" "$CODEX_HOME/config.toml" >/dev/null || {',
        '    printf "%s\\n" "CODEX_HOME config missing expected content" >&2',
        "    exit 7",
        "  }",
        "fi",
        'if [ -n "$AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN" ]; then',
        '  if grep -F -- "$AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN" "$CODEX_HOME/config.toml" >/dev/null; then',
        '    printf "%s\\n" "CODEX_HOME config contained forbidden content" >&2',
        "    exit 8",
        "  fi",
        "fi",
        'if [ -n "$AGENT_GROUP_FAKE_CODEX_STDERR" ]; then',
        '  printf "%s\\n" "$AGENT_GROUP_FAKE_CODEX_STDERR" >&2',
        "fi",
        'if [ -n "$output_path" ]; then',
        '  node -e \'const fs=require("node:fs"); const value=process.argv[2] ?? ""; fs.writeFileSync(process.argv[1], Buffer.from(value, "base64"));\' "$output_path" "${AGENT_GROUP_FAKE_CODEX_OUTPUT_B64:-e30=}"',
        "fi",
        'exit "${AGENT_GROUP_FAKE_CODEX_EXIT_CODE:-0}"',
        "",
      ].join("\n"),
    );
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-codex-text-" });
      const binDir = yield* makeFakeCodexBinary(tempDir);
      const previousPath = process.env.PATH;
      const previousAgentGroupHome = process.env.AGENT_GROUP_HOME;
      const previousOutput = process.env.AGENT_GROUP_FAKE_CODEX_OUTPUT_B64;
      const previousExitCode = process.env.AGENT_GROUP_FAKE_CODEX_EXIT_CODE;
      const previousStderr = process.env.AGENT_GROUP_FAKE_CODEX_STDERR;
      const previousRequireImage = process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_IMAGE;
      const previousStdinMustContain = process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_CONTAIN;
      const previousStdinMustNotContain = process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
      const previousRequireCodexHome = process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_CODEX_HOME;
      const previousRequireAuthJson = process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_AUTH_JSON;
      const previousRequireSkipGitRepoCheck =
        process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK;
      const previousRequireApprovalNever = process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_APPROVAL_NEVER;
      const previousCodexHomeConfigMustContain =
        process.env.AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN;
      const previousCodexHomeConfigMustNotContain =
        process.env.AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN;

      yield* Effect.sync(() => {
        process.env.PATH = `${binDir}:${previousPath ?? ""}`;
        process.env.AGENT_GROUP_HOME = tempDir;
        process.env.AGENT_GROUP_FAKE_CODEX_OUTPUT_B64 = Buffer.from(input.output, "utf8").toString(
          "base64",
        );

        if (input.exitCode !== undefined) {
          process.env.AGENT_GROUP_FAKE_CODEX_EXIT_CODE = String(input.exitCode);
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_EXIT_CODE;
        }

        if (input.stderr !== undefined) {
          process.env.AGENT_GROUP_FAKE_CODEX_STDERR = input.stderr;
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_STDERR;
        }

        if (input.requireImage) {
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_IMAGE = "1";
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_IMAGE;
        }

        if (input.stdinMustContain !== undefined) {
          process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_CONTAIN = input.stdinMustContain;
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_CONTAIN;
        }

        if (input.stdinMustNotContain !== undefined) {
          process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN = input.stdinMustNotContain;
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
        }

        if (input.requireCodexHome) {
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_CODEX_HOME = "1";
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_CODEX_HOME;
        }

        if (input.requireAuthJson) {
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_AUTH_JSON = "1";
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_AUTH_JSON;
        }

        if (input.requireSkipGitRepoCheck) {
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK = "1";
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK;
        }

        if (input.requireApprovalNever) {
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_APPROVAL_NEVER = "1";
        } else {
          delete process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_APPROVAL_NEVER;
        }
        ...
        if (input.codexHomeConfigMustContain !== undefined) {
          ...
          delete process.env.AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN;
        }
        ...
        if (input.codexHomeConfigMustNotContain !== undefined) {
          ...
          delete process.env.AGENT_GROUP_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN;
        }
      });

      return {
        previousPath,
        previousAgentGroupHome,
        previousOutput,
        previousExitCode,
        previousStderr,
    ...
    (previous) =>
      Effect.sync(() => {
        ...
        if (previous.previousAgentGroupHome === undefined) {
          delete process.env.AGENT_GROUP_HOME;
        } else {
          process.env.AGENT_GROUP_HOME = previous.previousAgentGroupHome;
        }

        if (previous.previousOutput === undefined) {
          delete process.env.AGENT_GROUP_FAKE_CODEX_OUTPUT_B64;
        } else {
          process.env.AGENT_GROUP_FAKE_CODEX_OUTPUT_B64 = previous.previousOutput;
        }

        if (previous.previousExitCode === undefined) {
          delete process.env.AGENT_GROUP_FAKE_CODEX_EXIT_CODE;
        } else {
          process.env.AGENT_GROUP_FAKE_CODEX_EXIT_CODE = previous.previousExitCode;
        }

        if (previous.previousStderr === undefined) {
          delete process.env.AGENT_GROUP_FAKE_CODEX_STDERR;
        } else {
          process.env.AGENT_GROUP_FAKE_CODEX_STDERR = previous.previousStderr;
        }

        if (previous.previousRequireImage === undefined) {
          delete process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_IMAGE;
        } else {
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_IMAGE = previous.previousRequireImage;
        }

        if (previous.previousStdinMustContain === undefined) {
          delete process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_CONTAIN;
        } else {
          process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_CONTAIN = previous.previousStdinMustContain;
        }

        if (previous.previousStdinMustNotContain === undefined) {
          delete process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
        } else {
          process.env.AGENT_GROUP_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN =
        ...
        if (previous.previousRequireCodexHome === undefined) {
          ...
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_CODEX_HOME = previous.previousRequireCodexHome;
        }

        if (previous.previousRequireAuthJson === undefined) {
          delete process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_AUTH_JSON;
        } else {
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_AUTH_JSON = previous.previousRequireAuthJson;
        }
        ...
        if (previous.previousRequireApprovalNever === undefined) {
          ...
          process.env.AGENT_GROUP_FAKE_CODEX_REQUIRE_APPROVAL_NEVER =
  ...
  it.effect("uses the provided codexHomePath and strips local skills config", () =>
    withFakeCodexEnv(
      ...
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const wrongCodexHome = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-wrong-codex-" });
        const customCodexHome = yield* fs.makeTempDirectoryScoped({
          prefix: "agent-group-custom-codex-",
        });
