...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "agent-group-git-core-test-",
});
  ...
  describe("stashAndCheckout", () => {
    ...
    it.effect("drops only the temporary stash it created after a successful reapply", () =>
      Effect.gen(function* () {
        ...
        expect(stashList).not.toContain("agent-group: stash before switching to feature");
        expect(yield* readTextFile(path.join(tmp, "README.md"))).toBe("dirty changes\n");
      }),
    );
    ...
    it.effect("keeps the stash when reapplying dirty changes conflicts", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["stash", "list"])).toContain(
          "agent-group: stash before switching to conflicting",
        );
      }),
    );
  });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "agent-group/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "agent-group/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "agent-group/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "agent-group/tmp-working",
          newBranch: "agent-group/feat/session",
        });

        expect(renamed.branch).toBe("agent-group/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "agent-group/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "agent-group/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("agent-group/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "agent-group/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "agent-group/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "agent-group/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "agent-group/tmp-working" });
        ...
        expect(renamed.branch).toBe("agent-group/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:example-org/agent-group.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:example-org/agent-group.git/",
          });
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, [
            "checkout",
            "-b",
            "agent-group/pr-488/statemachine",
            "--track",
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "agent-group/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
