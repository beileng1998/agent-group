...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const tempDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-cursor-text-acp-"));
      return {
        tempDir,
...
it.layer(CursorTextGenerationTestLayer)("CursorTextGenerationLive", (it) => {
  it.effect("uses ACP model config options instead of raw CLI model ids", () => {
    const requestLogDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-cursor-text-log-"));
    const requestLogPath = path.join(requestLogDir, "requests.ndjson");

    return withFakeAcpAgent(
      {
        AGENT_GROUP_ACP_REQUEST_LOG_PATH: requestLogPath,
        AGENT_GROUP_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("accepts json objects with extra assistant text around them", () =>
    withFakeAcpAgent(
      {
        AGENT_GROUP_ACP_PROMPT_RESPONSE_TEXT:
  ...
  it.effect("falls back to raw text when Cursor replies without JSON for a thread title", () =>
    withFakeAcpAgent(
      {
        AGENT_GROUP_ACP_PROMPT_RESPONSE_TEXT: "Sidebar Thread Row Spacing",
      },
      (agentPath) =>
        Effect.gen(function* () {
          const textGeneration = yield* TextGeneration;
  ...
  it.effect("recovers a thread title from a wrong-key JSON payload", () =>
    withFakeAcpAgent(
      {
        AGENT_GROUP_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({ name: "Reconnect Backoff Fix" }),
      },
      (agentPath) =>
        Effect.gen(function* () {
          const textGeneration = yield* TextGeneration;
  ...
  it.effect("closes the ACP child process after text generation completes", () => {
    const exitLogDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-cursor-text-exit-log-"));
    const exitLogPath = path.join(exitLogDir, "exit.log");

    return withFakeAcpAgent(
      {
        AGENT_GROUP_ACP_EXIT_LOG_PATH: exitLogPath,
        AGENT_GROUP_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
