...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousAgentGroupHome === undefined) delete process.env.AGENT_GROUP_HOME;
    else process.env.AGENT_GROUP_HOME = previousAgentGroupHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.AGENT_GROUP_HOME = "/agent-group-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/agent-group-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.agent-group/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.agent-group/runtime/codex-home-overlay";
