...
it.runIf(process.platform === "win32")(
  ...
  async () => {
    const root = mkdtempSync(Path.join(tmpdir(), "agent-group-effect-windows-process-"));
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "writeFileSync(process.env.AGENT_GROUP_CAPTURE_PATH, JSON.stringify(process.argv.slice(2)));",
          "",
        ].join("\n"),
      );
      writeFileSync(commandPath, `@echo off\r\n"${process.execPath}" "%~dp0capture.mjs" %*\r\n`);

      const env = { ...process.env, AGENT_GROUP_CAPTURE_PATH: outputPath };
