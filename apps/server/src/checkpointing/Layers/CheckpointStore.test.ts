...
import { CheckpointRef } from "@agent-group/contracts";
...
describe("CheckpointStoreLive", () => {
  ...
  it("deduplicates concurrent captures for the same checkpoint ref", async () => {
    ...
    await runtime.runPromise(
      Effect.gen(function* () {
        ...
        const input = {
          cwd: "/repo",
          checkpointRef: CheckpointRef.makeUnsafe("refs/agent-group-checkpoints/thread/message"),
        };
  ...
  it("skips the capture when skipIfExists is set and the ref already exists", async () => {
    const existingRef = "refs/agent-group-checkpoints/thread/existing";
    const missingRef = "refs/agent-group-checkpoints/thread/missing";
    const execute = vi.fn<GitCoreShape["execute"]>((input) => {
      const args = input.args.join(" ");
  ...
  it("restores the worktree patch when resetting the index fails during file undo", async () => {
    const fromRef = CheckpointRef.makeUnsafe("refs/agent-group-checkpoints/thread/turn/start");
    const toRef = CheckpointRef.makeUnsafe("refs/agent-group-checkpoints/thread/turn/end");
