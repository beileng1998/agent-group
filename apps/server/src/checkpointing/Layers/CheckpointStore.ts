  ...
  const captureCheckpointOnce: CheckpointStoreShape["captureCheckpoint"] = (input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "agent-group-fs-checkpoint-" }),
        (tempDir) =>
          Effect.gen(function* () {
            ...
            const commitEnv: NodeJS.ProcessEnv = {
              ...
              GIT_AUTHOR_NAME: "Agent Group",
              GIT_AUTHOR_EMAIL: "agent-group@users.noreply.github.com",
              GIT_COMMITTER_NAME: "Agent Group",
              GIT_COMMITTER_EMAIL: "agent-group@users.noreply.github.com",
            };
            ...
            const message = `Agent Group checkpoint ref=${input.checkpointRef}`;
  ...
  const reverseCheckpointDiff: CheckpointStoreShape["reverseCheckpointDiff"] = (input) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "agent-group-checkpoint-undo-" }),
        (tempDir) =>
          Effect.gen(function* () {
