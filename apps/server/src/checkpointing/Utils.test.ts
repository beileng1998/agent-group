...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Agent Group refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/agent-group\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/agent-group/", "refs/historical/");
