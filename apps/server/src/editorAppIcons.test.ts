...
describe("resolveCachedEditorIcon", () => {
  it("copies a macOS app PNG icon into the cache", async () => {
    const homeDir = makeTempDir("agent-group-editor-icon-home-");
    const cacheDir = makeTempDir("agent-group-editor-icon-cache-");
  ...
  it("resolves a Linux desktop icon by icon name", async () => {
    const homeDir = makeTempDir("agent-group-editor-icon-linux-home-");
    const cacheDir = makeTempDir("agent-group-editor-icon-linux-cache-");
  ...
  it("copies a Windows Store package PNG icon for VS Code", async () => {
    const programFilesDir = makeTempDir("agent-group-editor-icon-win-program-files-");
    const cacheDir = makeTempDir("agent-group-editor-icon-win-cache-");
    const powershellBinDir = makeTempDir("agent-group-editor-icon-win-powershell-");
    const localAppData = makeTempDir("agent-group-editor-icon-win-local-appdata-");
  ...
  it("does not match Linux desktop files from unrelated comments", async () => {
    const homeDir = makeTempDir("agent-group-editor-icon-linux-comment-home-");
    const cacheDir = makeTempDir("agent-group-editor-icon-linux-comment-cache-");
    writeFakeLinuxDesktopIcon({
      homeDir,
  ...
  it("does not match short Linux editor ids inside unrelated words", async () => {
    const homeDir = makeTempDir("agent-group-editor-icon-linux-short-home-");
    const cacheDir = makeTempDir("agent-group-editor-icon-linux-short-cache-");
    writeFakeLinuxDesktopIcon({
      homeDir,
  ...
  it("short-circuits repeated missing native icon lookups briefly", async () => {
    const homeDir = makeTempDir("agent-group-editor-icon-linux-negative-home-");
    const cacheDir = makeTempDir("agent-group-editor-icon-linux-negative-cache-");
    const lookup = {
      editorId: "ghostty",
      cacheDir,
  ...
  it("returns null for unknown editor ids", async () => {
    await expect(
      resolveCachedEditorIcon({
        ...
        cacheDir: makeTempDir("agent-group-editor-icon-missing-cache-"),
        platform: "darwin",
        env: { HOME: makeTempDir("agent-group-editor-icon-missing-home-"), PATH: "" },
      }),
    ).resolves.toBeNull();
  });
});
