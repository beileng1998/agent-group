...
import { fileExists, git, sessionRef, temporaryWorkspace } from "./runtime.testSupport";
...
describe("Agent Group context runtime", () => {
  ...
  it("uses global prompt instructions for every Group turn", async () => {
    ...
    const prepared = await prepareAgentGroupTurn({
      ...input,
      userText: "Continue.",
      mentionedSessions: [{ sessionId: ThreadId.makeUnsafe("mentioned-custom"), title: "Custom" }],
      globalSettings: {
        ...DEFAULT_SERVER_SETTINGS.agentGroup,
        promptInstructions,
      },
  ...
  it("suspends the context lifecycle while global Agent Group context is disabled", async () => {
    ...
    expect(resumed?.prompt).toContain('<rules scope="group">\nShared group rules.\n</rules>');
