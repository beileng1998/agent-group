...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-terminal-"));
  ...
  it("keeps a running terminal alive when open reattaches with different cwd or env", async () => {
    ...
    const snapshot = await manager.open(
      openInput({ cwd: logsDir, env: { AGENT_GROUP_TERMINAL_TEST: "changed" } }),
    );
  ...
  it("keeps incomplete-tree fallback escalation after root exit without re-signaling the root", async () => {
    ...
    const processTreeKiller: ProcessTreeKiller = {
      ...
      signal: ({ rootPid, signal, tree, includeRootTree, allowLegacyTreeFallback }) => {
        treeSignals.push({
          rootPid,
          signal,
          descendantPids: tree.descendants.map((descendant) => descendant.pid),
          includeRootTree,
          allowLegacyTreeFallback,
        });
      },
    };
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.AGENT_GROUP_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          AGENT_GROUP_PROJECT_ROOT: "/repo",
          AGENT_GROUP_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.AGENT_GROUP_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.AGENT_GROUP_WORKTREE_PATH).toBe("/repo/worktree-a");
