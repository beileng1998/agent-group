import { AuthSessionId } from "@agent-group/contracts";
...
const makeAuthSessionRepository = Effect.gen(function* () {
  ...
  const setExpiresAtRows = SqlSchema.findAll({
    Request: SetAuthSessionExpiresAtInput,
  ...
  const setExpiresAt: AuthSessionRepositoryShape["setExpiresAt"] = (input) =>
    setExpiresAtRows(input as unknown as Parameters<typeof setExpiresAtRows>[0]).pipe(
      Effect.mapError(
        toPersistenceSqlOrDecodeError(
          "AuthSessionRepository.setExpiresAt:query",
          "AuthSessionRepository.setExpiresAt:decodeRows",
        ),
      ),
      Effect.map((rows) => rows.length > 0),
    );
