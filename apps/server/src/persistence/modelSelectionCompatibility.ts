...
import { MODEL_OPTIONS_BY_PROVIDER } from "@agent-group/contracts";
...
// Imported instance ids may be runtime names rather than Agent Group provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Agent Group writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Agent Group stores canonical provider/options objects.
