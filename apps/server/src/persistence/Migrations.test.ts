...
layer("reconcileMigrationLineage", (it) => {
  ...
  // mark is at or beyond Agent Group's latest migration ID. The migrator's max-ID
  // gate then skips every Agent Group migration — including the #032 self-heal —
  // and startup crashes on the missing env_mode column.
  it.effect("re-runs skipped migrations when an imported tracker outruns Agent Group's latest ID", () =>
    Effect.gen(function* () {
      ...
      // Record a foreign lineage from 17 through past Agent Group's latest ID.
      const latestAgentGroupId = Math.max(...migrationEntries.map(([id]) => id));
      for (let id = 17; id <= latestAgentGroupId + 3; id++) {
        yield* sql`
      ...
      // The tracker now mirrors the Agent Group lineage exactly; foreign rows are gone.
  ...
  it.effect("preserves tracker rows written by a newer Agent Group build", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES (${futureId}, 'FutureAgentGroupMigration')
      `;
      ...
      assert.deepStrictEqual(rows[rows.length - 1], {
        ...
        name: "FutureAgentGroupMigration",
      });
    }),
  );
