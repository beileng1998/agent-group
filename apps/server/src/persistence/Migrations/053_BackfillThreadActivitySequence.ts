...
export default Effect.gen(function* () {
  ...
  yield* sql`DROP TABLE IF EXISTS temp_agent_group_activity_sequences`;

  yield* sql`
    CREATE TEMP TABLE temp_agent_group_activity_sequences (
  ...
  yield* sql`
    INSERT INTO temp_agent_group_activity_sequences (activity_id, sequence)
    SELECT
  ...
  yield* sql`
    ...
    SET sequence = (
      SELECT temp_agent_group_activity_sequences.sequence
      FROM temp_agent_group_activity_sequences
      WHERE temp_agent_group_activity_sequences.activity_id = projection_thread_activities.activity_id
    )
  ...
  yield* sql`DROP TABLE temp_agent_group_activity_sequences`;
});
