...
layer("032_ReconcileImportedSchemaLineage", (it) => {
  // Simulates a legacy ~/.agent-group import where the imported `effect_sql_migrations`
  // tracker has IDs 17-31 recorded under unrelated Agent Group names. The 17-31
  ...
  // references env_mode.
  it.effect("heals an imported Agent Group DB whose tracker skipped 17-31", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      // Bring the schema to where Agent Group and Agent Group last agreed.
      yield* runMigrations({ toMigrationInclusive: 16 });

      // Mark IDs 17-31 applied under Agent Group's old names so the migrator
      // skips Agent Group's renumbered 17-23. Names are illustrative; only the
      ...
      // Seed a thread row with the Agent Group-era column set so the data-rewrite
  ...
  it.effect("is a no-op on a fresh Agent Group install", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
