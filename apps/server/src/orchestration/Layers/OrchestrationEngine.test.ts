...
async function createOrchestrationSystem() {
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "agent-group-orchestration-engine-test-",
  });
  ...
  it("stores completed checkpoint summaries even when no files changed", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        checkpointRef: asCheckpointRef("refs/agent-group/checkpoints/thread-turn-diff/turn/1"),
        status: "ready",
        files: [],
  ...
  it("rejects duplicate Studio workspace containers", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/agent-group-studio",
  ...
  it("rejects Studio and regular projects claiming each other's workspace root", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/agent-group-cross-kind-studio",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    await system.run(
      engine.dispatch({
        ...
        kind: "project",
        title: "App",
        workspaceRoot: "/tmp/agent-group-cross-kind-app",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    ...
    await expect(
      system.run(
        engine.dispatch({
          ...
          projectId: asProjectId("project-cross-kind-app"),
          kind: "studio",
          workspaceRoot: "/tmp/agent-group-cross-kind-pinned-studio",
        }),
      ),
  ...
  it("rejects moving a Studio container onto another Studio workspace root", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/agent-group-studio-source",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/agent-group-studio-target",
        defaultModelSelection: null,
        createdAt,
      }),
    );
