...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("bounds large tool activity data while keeping command metadata", async () => {
    ...
    expect(data.__agentGroupTruncated).toBe(true);
  ...
  it("ignores late events from the provider replaced at a turn boundary", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.makeUnsafe("cmd-session-switching-to-claude"),
        threadId: asThreadId("thread-1"),
        session: {
          threadId: asThreadId("thread-1"),
          status: "starting",
    ...
    harness.emit({
      type: "runtime.error",
      eventId: asEventId("evt-stale-codex-after-provider-switch"),
      provider: "codex",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-old-codex"),
      payload: { message: "old runtime exited" },
    });
    await harness.drain();
    ...
    expect(
      thread?.activities.some(
        (activity) => activity.id === "evt-stale-codex-after-provider-switch",
      ),
    ).toBe(false);
  });
