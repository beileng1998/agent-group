...
import { HighlightsQueryLive } from "./HighlightsQuery.ts";
import { OrchestrationEngineService } from "../Services/OrchestrationEngine.ts";
import { ProjectionSnapshotQuery } from "../Services/ProjectionSnapshotQuery.ts";
import { HighlightsQuery } from "../Services/HighlightsQuery.ts";

async function createSystem() {
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "agent-group-pinned-roundtrip-test-",
  });
  const layer = OrchestrationEngineLive.pipe(
    Layer.provideMerge(OrchestrationProjectionPipelineLive),
    Layer.provideMerge(OrchestrationProjectionSnapshotQueryLive),
    Layer.provideMerge(HighlightsQueryLive),
  ...
  const highlights = await runtime.runPromise(Effect.service(HighlightsQuery));
  return {
    engine,
    query,
    highlights,
...
describe("pinned messages round-trip", () => {
  it("persists pinned-message commands into projected thread detail and snapshots", async () => {
    ...
    try {
      ...
      const savedItems = await system.run(
        system.highlights.list({
          scope: { type: "session", sessionId: threadId },
          query: "Review this answer",
          colors: [],
          kinds: [],
          noteFilter: "all",
          cursor: null,
      ...
      expect(savedItems.items).toHaveLength(1);
      expect(savedItems.items[0]).toMatchObject({
        kind: "pin",
        pin: { messageId, label: "Review this answer", done: true },
        group: { id: projectId, title: "Pins project" },
        session: { id: threadId, title: "Pins thread" },
        message: { id: messageId, exists: false, text: "" },
      });
    } finally {
    ...
    try {
      ...
      await system.run(
        system.engine.dispatch({
          ...
          commandId: CommandId.makeUnsafe("cmd-marker-message"),
          threadId,
          message: {
            messageId,
            role: "user",
            text: "hello important text today xx underline",
            attachments: [],
          },
          runtimeMode: "approval-required",
          interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
      ...
      await expect(
        system.run(
          system.engine.dispatch({
            type: "thread.marker.add",
            commandId: CommandId.makeUnsafe("cmd-marker-overlap"),
            threadId,
            markerId: ThreadMarkerId.makeUnsafe("marker-overlap"),
            messageId,
            startOffset: 10,
            endOffset: 20,
            selectedText: "rtant text",
            style: "highlight",
            color: "pink",
          }),
        ),
      ...
      await system.run(
        system.engine.dispatch({
          ...
          commandId: CommandId.makeUnsafe("cmd-marker-note"),
          threadId,
          markerId: firstMarkerId,
          note: "**Follow up** with the source.",
        }),
      );
      await system.run(
        system.engine.dispatch({
      ...
      const highlights = await system.run(
        system.highlights.list({
          scope: { type: "session", sessionId: threadId },
          query: "important",
          colors: [],
      ...
      expect(highlights.items).toHaveLength(1);
      expect(highlights.items[0]).toMatchObject({
        kind: "highlight",
        marker: { id: firstMarkerId, note: "**Follow up** with the source." },
        group: { id: projectId, title: "Markers project" },
        session: { id: threadId, title: "Markers thread" },
        message: { id: messageId, exists: true },
      });
