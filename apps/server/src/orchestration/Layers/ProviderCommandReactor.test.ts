...
import { getAgentGroupSession } from "../../agentGroup/runtime.ts";
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    readonly agentGroupSettings?: AgentGroupServerSettings;
    ...
    const baseDir =
      input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-reactor-"));
    createdBaseDirs.add(baseDir);
    const workspaceRoot = input?.workspaceRoot ?? path.join(baseDir, "workspace");
    ...
    const resolveTranscriptPath = vi.fn<NonNullable<ProviderServiceShape["resolveTranscriptPath"]>>(
      input?.resolveTranscriptPath ?? (() => Effect.succeed(null)),
    );
  ...
  it("stops an active provider runtime and immediately resends an edited latest message", async () => {
    ...
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
      threadId: ThreadId.makeUnsafe("thread-1"),
      input: expect.stringContaining("<user_request>\nedited prompt\n</user_request>"),
      attachments: [imageAttachment],
      skills: [skill],
    });
    expect(harness.sendTurn.mock.calls[0]?.[0].input).not.toContain("<mentioned_sessions>");
    expect(harness.sendTurn.mock.calls[0]?.[0].mentions).toBeUndefined();
  ...
  it("sends the Agent Group protocol prompt without replaying the retained transcript", async () => {
    const now = new Date().toISOString();
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-reactor-"));
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.messages.import",
        commandId: CommandId.makeUnsafe("cmd-import-agent-group-history"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        messages: [
          {
            messageId: asMessageId("agent-group-old-user"),
            role: "user",
            text: "Old transcript must stay out of the provider prompt.",
            createdAt: now,
            updatedAt: now,
          },
          {
            messageId: asMessageId("agent-group-old-assistant"),
            role: "assistant",
            text: "Old assistant reply must also stay out.",
            createdAt: now,
            updatedAt: now,
          },
        ],
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-agent-group-first-turn"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("agent-group-first-user"),
          role: "user",
          text: "  Do the work.  ",
          attachments: [],
        },
    ...
    expect(sent?.input).toBe(
      [
        "<user_request>\n  Do the work.  \n</user_request>",
        '<session_context path=".agent-group/sessions/thread-1/context.md">\nProactively maintain this file as the current Session context.\n</session_context>',
      ].join("\n\n"),
    );
    expect(sent?.input).not.toContain("<thread_context>");
  ...
  it("sends original provider input when Agent Group context is disabled", async () => {
    const now = new Date().toISOString();
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-disabled-"));
    createdBaseDirs.add(workspaceRoot);
    const coordinates = {
      workspaceRoot,
      groupId: asProjectId("project-1"),
      sessionId: ThreadId.makeUnsafe("thread-1"),
      createdAt: now,
    };
    const harness = await createHarness({
      workspaceRoot,
      agentGroupSettings: {
        ...
        globalRules: "This must never reach the provider.",
      },
    });
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-agent-group-disabled-turn"),
        threadId: coordinates.sessionId,
        message: {
          messageId: asMessageId("agent-group-disabled-user"),
          role: "user",
          text: "  Send this unchanged.  ",
          attachments: [],
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
    ...
    await waitFor(() => harness.sendTurn.mock.calls.length === 1);
    const sent = harness.sendTurn.mock.calls[0]?.[0] as { readonly input?: string } | undefined;
    expect(sent?.input).toBe("Send this unchanged.");
    expect(sent?.input).not.toContain("This must never reach the provider.");
    expect(sent?.input).not.toContain("<session_context");
  });

  it("injects validated same-group session mentions and keeps them out of provider mentions", async () => {
    const now = new Date().toISOString();
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-mentions-"));
    createdBaseDirs.add(workspaceRoot);
    const transcriptPath = path.join(workspaceRoot, "research-transcript.jsonl");
    fs.writeFileSync(transcriptPath, "{}\n", "utf8");
    const researchId = ThreadId.makeUnsafe("thread-research");
    const harness = await createHarness({
      workspaceRoot,
      resolveTranscriptPath: ({ threadId }) =>
        Effect.succeed(threadId === researchId ? transcriptPath : null),
    });

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.create",
        commandId: CommandId.makeUnsafe("cmd-create-research-session"),
        threadId: researchId,
        projectId: asProjectId("project-1"),
    ...
    const otherSessionId = ThreadId.makeUnsafe("thread-other-group");
    const otherWorkspace = path.join(workspaceRoot, "other-group");
    fs.mkdirSync(otherWorkspace, { recursive: true });
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "project.create",
        commandId: CommandId.makeUnsafe("cmd-create-other-group"),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.create",
        commandId: CommandId.makeUnsafe("cmd-create-other-group-session"),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-mention-research-session"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("message-mention-research"),
          role: "user",
          text: 'Use @"Research notes" and @GitHub.',
          attachments: [],
          mentions: [
            {
              kind: "session",
              sessionId: researchId,
              name: "Untrusted browser title",
            },
            { name: "GitHub", path: "plugin://github@curated" },
            {
              kind: "session",
              sessionId: ThreadId.makeUnsafe("missing-session"),
              name: "Missing",
            },
            {
              kind: "session",
              sessionId: otherSessionId,
              name: "Other group session",
            },
          ],
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
    ...
    const sent = harness.sendTurn.mock.calls[0]?.[0] as
      | { readonly input?: string; readonly mentions?: unknown }
      | undefined;
    expect(sent?.input).toContain("<mentioned_sessions>");
    expect(sent?.input).toContain('"session_id":"thread-research"');
    expect(sent?.input).toContain('"title":"Research notes"');
    expect(sent?.input).toContain(
      '"context_path":".agent-group/sessions/thread-research/context.md"',
    );
    expect(sent?.input).toContain(`"transcript_path":"${transcriptPath}"`);
    expect(sent?.input).not.toContain("Untrusted browser title");
    expect(sent?.input).not.toContain("missing-session");
    expect(sent?.input).not.toContain("thread-other-group");
    expect(sent?.mentions).toEqual([{ name: "GitHub", path: "plugin://github@curated" }]);
    expect(
      fs.existsSync(path.join(workspaceRoot, ".agent-group/sessions/thread-research/context.md")),
    ).toBe(true);
    expect(harness.resolveTranscriptPath).toHaveBeenCalledWith({ threadId: researchId });
  });

  it("finalizes a successful Agent Group turn before draining the next queued turn", async () => {
    const now = new Date().toISOString();
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-reactor-"));
    createdBaseDirs.add(workspaceRoot);
    await getAgentGroupSession({
      workspaceRoot,
      groupId: asProjectId("project-1"),
      sessionId: ThreadId.makeUnsafe("thread-1"),
      createdAt: now,
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-agent-group-active-turn"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("agent-group-active-user"),
          role: "user",
          text: "First task.",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.makeUnsafe("cmd-agent-group-session-running"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        session: {
          threadId: ThreadId.makeUnsafe("thread-1"),
          status: "running",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-agent-group-queued-turn"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("agent-group-queued-user"),
          role: "user",
          text: "Second task.",
          attachments: [],
        },
    ...
    fs.writeFileSync(
      path.join(workspaceRoot, ".agent-group/sessions/thread-1/context.md"),
      "# Updated by the first turn\n",
    );
    harness.setRuntimeSessionTurnState({ threadId: "thread-1", status: "ready" });
    await harness.emitRuntimeEvent({
      type: "turn.completed",
      eventId: asEventId("evt-agent-group-turn-completed"),
      provider: "codex",
      threadId: ThreadId.makeUnsafe("thread-1"),
      createdAt: new Date().toISOString(),
    ...
    expect(sent?.input).not.toContain("Read before work.");
    const state = JSON.parse(
      fs.readFileSync(path.join(workspaceRoot, ".agent-group/state.json"), "utf8"),
    ) as { readonly sessions: Record<string, { readonly firstTurnCompleted: boolean }> };
    expect(state.sessions["thread-1"]?.firstTurnCompleted).toBe(true);
  });
  ...
  it("clears stale Claude resume state and retries the turn with transcript context", async () => {
    ...
    expect(retrySendInput.input).toContain("<session_context");
  ...
  it("always starts providers in the canonical project workspace root", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
  ...
  it("renames a temporary sidechat from its first question and preserves the prefix", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    const sidechatThreadId = ThreadId.makeUnsafe("thread-sidechat-title");
    const assistantMessageId = asMessageId("sidechat-title-source-assistant");
    const messageText = [
      "How should this rule be applied here?",
      "",
      "<assistant_selection>",
      `- assistant message ${assistantMessageId}:`,
      "  Cache invalidation happens after writes.",
      "</assistant_selection>",
    ].join("\n");
    harness.generateThreadTitle.mockImplementation(() =>
      Effect.succeed({ title: "Apply Cache Invalidation Rule" }),
    );

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.fork.create",
        commandId: CommandId.makeUnsafe("cmd-sidechat-title-create"),
        threadId: sidechatThreadId,
        sourceThreadId: ThreadId.makeUnsafe("thread-1"),
        sidechatSourceThreadId: ThreadId.makeUnsafe("thread-1"),
        projectId: asProjectId("project-1"),
        // Compatibility with Side drafts created before the unified placeholder.
        title: "Sidechat: Cache invalidation happens after writes",
        modelSelection: {
          provider: "codex",
          model: "gpt-5-codex",
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-sidechat-title-turn"),
        threadId: sidechatThreadId,
        message: {
          messageId: asMessageId("sidechat-title-user"),
          role: "user",
          text: messageText,
          attachments: [
            {
              type: "assistant-selection",
              id: "sidechat-title-selection",
              assistantMessageId,
              text: "Cache invalidation happens after writes.",
            },
          ],
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
    ...
    await waitFor(async () => {
      const readModel = await Effect.runPromise(harness.engine.getReadModel());
      return (
        readModel.threads.find((thread) => thread.id === sidechatThreadId)?.title ===
        "Sidechat: Apply Cache Invalidation Rule"
      );
    });
  });
  ...
  it("renames temporary worktree branches and keeps associated worktree metadata in sync", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        threadId: ThreadId.makeUnsafe("thread-1"),
        envMode: "worktree",
        branch: "agent-group/cb661f0d",
        worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
        associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
        associatedWorktreeBranch: "agent-group/cb661f0d",
        associatedWorktreeRef: "agent-group/cb661f0d",
      }),
    );
    ...
    await waitFor(async () => {
      ...
      return (
        thread?.branch === "agent-group/app-startup-crash" &&
        thread.associatedWorktreeBranch === "agent-group/app-startup-crash" &&
        thread.associatedWorktreeRef === "agent-group/app-startup-crash"
      );
    });
    ...
    expect(thread).toMatchObject({
      branch: "agent-group/app-startup-crash",
      worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreeBranch: "agent-group/app-startup-crash",
      associatedWorktreeRef: "agent-group/app-startup-crash",
    });
  });
  ...
  it("falls back to prompt-based worktree branch names when the provider cannot generate one", async () => {
    ...
    expect(harness.renameBranch.mock.calls[0]?.[0]).toMatchObject({
      oldBranch: "agent-group/cb661f0d",
      newBranch: "agent-group/fix-provider-startup-timeouts",
    });
    ...
    await waitFor(async () => {
      ...
      return thread?.branch === "agent-group/fix-provider-startup-timeouts";
    });
  });
  ...
  it("waits for the current turn to settle before switching providers", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    harness.setRuntimeSessionTurnState({
      threadId: "thread-1",
      status: "running",
      activeTurnId: asTurnId("turn-before-provider-switch"),
    });
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.makeUnsafe("cmd-session-running-before-provider-switch"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        session: {
          threadId: ThreadId.makeUnsafe("thread-1"),
          status: "running",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-queued-provider-switch"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("msg-queued-provider-switch"),
          role: "user",
    ...
    await harness.emitRuntimeEvent({
      type: "turn.completed",
      eventId: asEventId("evt-before-provider-switch-completed"),
      provider: "codex",
      threadId: ThreadId.makeUnsafe("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: asTurnId("turn-before-provider-switch"),
      payload: { state: "completed" },
      providerRefs: {},
    } as ProviderRuntimeEvent);
  ...
  it("switches providers at the turn boundary without resuming the old session", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.meta.update",
        commandId: CommandId.makeUnsafe("cmd-select-provider-switch-2"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        modelSelection: {
          provider: "claudeAgent",
          model: "claude-opus-4-6",
    ...
    expect(harness.stopSession.mock.invocationCallOrder[0]).toBeLessThan(
      harness.startSession.mock.invocationCallOrder[1] ?? Number.POSITIVE_INFINITY,
    );
    expect(harness.startSession.mock.calls[1]?.[1]).toMatchObject({
      provider: "claudeAgent",
      cwd: harness.workspaceRoot,
  ...
  it("replays the Session transcript when switching from Codex to Pi", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-codex-before-pi"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-codex-before-pi"),
          role: "user",
          text: "Remember alpha.",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.makeUnsafe("cmd-codex-reply-before-pi"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        messageId: asMessageId("assistant-codex-before-pi"),
        delta: "Alpha saved.",
        turnId: asTurnId("turn-1"),
        createdAt: now,
      }),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.message.assistant.complete",
        commandId: CommandId.makeUnsafe("cmd-codex-reply-complete-before-pi"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        messageId: asMessageId("assistant-codex-before-pi"),
        turnId: asTurnId("turn-1"),
        createdAt: now,
    ...
    const beforeSwitch = await Effect.runPromise(harness.engine.getReadModel());
    const storedThread = beforeSwitch.threads.find(
      (entry) => entry.id === ThreadId.makeUnsafe("thread-1"),
    );
    expect(storedThread?.messages).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: "user", text: "Remember alpha." }),
        expect.objectContaining({ role: "assistant", text: "Alpha saved." }),
      ]),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.meta.update",
        commandId: CommandId.makeUnsafe("cmd-select-pi-after-codex"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        modelSelection: {
          provider: "pi",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-pi-after-codex"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-pi-after-codex"),
          role: "user",
          text: "What did we save?",
          attachments: [],
        },
        modelSelection: {
          provider: "pi",
    ...
    const piTurn = harness.sendTurn.mock.calls[1]?.[0] as { readonly input?: string } | undefined;
    expect(piTurn?.input).toContain("<thread_context>");
    expect(piTurn?.input).toContain("Remember alpha.");
    expect(piTurn?.input).toContain("Alpha saved.");
    expect(piTurn?.input).toContain("What did we save?");
    expect(piTurn?.input).not.toContain("Agent Group");
    expect(piTurn?.input?.match(/What did we save\?/g)).toHaveLength(1);

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-pi-continuation"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-pi-continuation"),
          role: "user",
          text: "Continue with Pi.",
          attachments: [],
        },
        modelSelection: {
          provider: "pi",
    ...
    await waitFor(() => harness.sendTurn.mock.calls.length === 3);
    const piContinuation = harness.sendTurn.mock.calls[2]?.[0] as
      | { readonly input?: string }
      | undefined;
    expect(piContinuation?.input).not.toContain("<thread_context>");
    expect(piContinuation?.input).toContain("Continue with Pi.");
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.meta.update",
        commandId: CommandId.makeUnsafe("cmd-select-codex-after-runtime-loss"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        modelSelection: {
          provider: "codex",
          model: "gpt-5-codex",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-codex-after-runtime-loss"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-codex-after-runtime-loss"),
          role: "user",
          text: "Back to Codex?",
          attachments: [],
        },
    ...
    const recoveredCodexTurn = harness.sendTurn.mock.calls[3]?.[0] as
      | { readonly input?: string }
      | undefined;
    expect(recoveredCodexTurn?.input).toContain("<thread_context>");
    expect(recoveredCodexTurn?.input).toContain("Alpha saved.");
    expect(recoveredCodexTurn?.input?.match(/Back to Codex\?/g)).toHaveLength(1);
  });
  ...
  it("keeps user-created child sessions on their own provider binding", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    const childThreadId = ThreadId.makeUnsafe("thread-user-child");

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.create",
        commandId: CommandId.makeUnsafe("cmd-thread-create-user-child"),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.makeUnsafe("cmd-session-set-user-child"),
        threadId: childThreadId,
        session: {
          threadId: childThreadId,
          status: "running",
          providerName: "claudeAgent",
          runtimeMode: "approval-required",
          activeTurnId: asTurnId("turn-user-child"),
          lastError: null,
          updatedAt: now,
        },
        createdAt: now,
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.interrupt",
        commandId: CommandId.makeUnsafe("cmd-turn-interrupt-user-child"),
        threadId: childThreadId,
        createdAt: now,
      }),
    );
    ...
    await waitFor(() => harness.interruptTurn.mock.calls.length === 1);
    expect(harness.interruptTurn.mock.calls[0]?.[0]).toEqual({
      threadId: "thread-user-child",
      turnId: "turn-user-child",
    });
  });
