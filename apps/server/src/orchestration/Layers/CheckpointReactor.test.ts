...
import type { ProviderKind, ProviderRuntimeEvent, ProviderSession } from "@agent-group/contracts";
import {
  CheckpointRef,
  CommandId,
...
function createGitRepository(hasInitialCommit = true) {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-checkpoint-handler-"));
...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
      prefix: "agent-group-checkpoint-reactor-test-",
    });
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = fs.mkdtempSync(
      path.join(os.tmpdir(), "agent-group-checkpoint-runtime-non-repo-"),
    );
  ...
  it("restores turn zero from the persisted checkpoint family", async () => {
    ...
    const historicalTurnOneRef = CheckpointRef.makeUnsafe(
      checkpointRefForThreadTurn(threadId, 1).replace("refs/agent-group/", "refs/historical/"),
    );
