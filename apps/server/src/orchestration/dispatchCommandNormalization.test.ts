...
function projectCreateCommand(
  ...
  return {
    ...
    kind: "chat",
    title: "Chat",
    workspaceRoot: "/Users/tester/Documents/Agent Group/2026-06-11/chat",
...
describe("makeDispatchCommandNormalizer", () => {
  it("returns a deferred prepare effect instead of scaffolding during normalization", async () => {
    ...
    expect(preparedRoots).toEqual(["/Users/tester/Documents/Agent Group/2026-06-11/chat"]);
  });
  ...
  it("does not prepare ordinary projects or the chat workspace root itself", async () => {
    ...
    const first = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          workspaceRoot: "/Users/tester/Documents/Agent Group/2026-06-11/app",
        }),
      }),
    );
    ...
    const second = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          workspaceRoot: "/Users/tester/Documents/Agent Group",
        }),
      }),
    );
  ...
  it("prepares the Studio workspace root itself", async () => {
    ...
    expect(preparedRoots).toEqual(["/Users/tester/Documents/Agent Group/Studio"]);
  });
  ...
  it("prepares nested Studio workspace roots but not ordinary projects under Studio", async () => {
    ...
    const first = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "studio",
          workspaceRoot: "/Users/tester/Documents/Agent Group/Studio/Outbox",
        }),
      }),
    );
    await runPrepareWorkspaceRoot(first);
    const second = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          workspaceRoot: "/Users/tester/Documents/Agent Group/Studio/SomeProject",
        }),
      }),
    );
    await runPrepareWorkspaceRoot(second);

    expect(preparedRoots).toEqual(["/Users/tester/Documents/Agent Group/Studio/Outbox"]);
  });

  it("rolls back attachment files written before a later upload fails", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-group-dispatch-normalize-"));
    const fileSystem = {
