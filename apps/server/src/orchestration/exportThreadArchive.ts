...
// Purpose: Build a ZIP archive that exports a single Agent Group thread so a user
...
// The export-eligibility guard lives in @agent-group/shared/threadExport so the
...
export function threadArchiveFileName(input: {
  ...
  return `agent-group-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
