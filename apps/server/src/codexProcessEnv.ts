...
// Purpose: Builds the exact environment used when Agent Group launches Codex subprocesses.
...
import { readActiveCodexProviderEnvKey } from "@agent-group/shared/codexConfig";
import {
  readEnvironmentFromLoginShell,
  resolveLoginShell,
  type ShellEnvironmentReader,
} from "@agent-group/shared/shell";

import { resolveBaseCodexHomePath, resolveAgentGroupCodexHomeOverlayPath } from "./codexHomePaths.ts";
...
const AGENT_GROUP_CONFIG_SUPPRESSIONS_FILE = "agent-group-config-suppressions-v1.json";
...
export function resolveCodexBrowserUsePipePath(
  ...
  const configured = env.AGENT_GROUP_BROWSER_USE_PIPE_PATH?.trim();
  if (configured) {
    return configured;
  }
...
export function readAgentGroupConfigSuppressions(markerPath: string): readonly string[] {
  try {
...
function writeAgentGroupConfigSuppressions(
  markerPath: string,
  sectionHeaders: readonly string[],
): void {
...
function prepareAgentGroupCodexHomeOverlay(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
}): string | undefined {
  const sourceHomePath = resolveBaseCodexHomePath(input.env, input.homePath);
  const overlayHomePath = resolveAgentGroupCodexHomeOverlayPath(input.env, sourceHomePath);
  ...
  const suppressionMarkerPath = path.join(overlayHomePath, AGENT_GROUP_CONFIG_SUPPRESSIONS_FILE);
  const suppressedSections = [
    ...new Set([
      ...findConflictingLocalBrowserPluginSections(sourceConfig),
      ...readAgentGroupConfigSuppressions(suppressionMarkerPath),
    ]),
  ...
  writeAgentGroupConfigSuppressions(suppressionMarkerPath, suppressedSections);
...
export function buildCodexProcessEnv(
  ...
  const overlayHomePath = prepareAgentGroupCodexHomeOverlay({
    env: baseEnv,
