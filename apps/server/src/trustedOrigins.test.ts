...
describe("trustedOrigins", () => {
  ...
  it("normalizes desktop origins with trailing slashes", () => {
    expect(normalizeCorsOrigin("agent-group://app/")).toBe("agent-group://app");
  });
  ...
  it("requires a reverse proxy origin to match its forwarded HTTPS scheme", () => {
    expect(
      shouldRejectUntrustedRequestOrigin({
        rawOrigin: "http://127.0.0.1:58090",
        requestOrigin: "https://127.0.0.1:58090",
        config,
      }),
    ).toBe(true);
    expect(
      shouldRejectUntrustedRequestOrigin({
        rawOrigin: "https://127.0.0.1:58090",
        requestOrigin: "https://127.0.0.1:58090",
        config,
      }),
    ).toBe(false);
