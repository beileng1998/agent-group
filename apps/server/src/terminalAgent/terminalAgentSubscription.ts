...
function isSnapshotTooLarge(cause: unknown, seen = new Set<unknown>()): boolean {
  ...
  return (
    candidate.reason === "snapshot-too-large" ||
    isSnapshotTooLarge(candidate.cause, seen)
  );
}
...
export function makeTerminalAgentSubscription(
  ...
  return Stream.callback<TerminalAgentEvent, TerminalAgentServiceError>((queue) =>
    Effect.gen(function* () {
      let unsubscribeClient: (() => void) | null = null;
      let attachedEpoch: { revision: number; generation: string } | null = null;
      let blockedAttachEpoch: { revision: number; generation: string } | null =
        null;
      ...
      const offer = (event: TerminalAgentEvent) =>
        Queue.offer(queue, event).pipe(
          Effect.flatMap((accepted) =>
            accepted
              ? Effect.void
              : Queue.fail(queue, backpressureError()).pipe(Effect.asVoid),
          ),
        );
      const offerFromCallback = (event: TerminalAgentEvent, token: symbol) => {
        if (callbackOverflowed) return;
        const eventBytes =
          event.type === "output" ? Buffer.byteLength(event.data) : 0;
          ...
          if (attached._tag === "Failure") {
            ...
            yield* offer(
              stateEvent(
                marked._tag === "Success"
                  ? marked.success
                  : yield* dependencies.coordinator.getState(
      ...
      const pullChanges = yield* dependencies.coordinator.streamChanges.pipe(
        Stream.filter(
          (change: ExecutionAdapterChange) =>
            change.threadId === dependencies.threadId,
        ),
        Stream.toPull,
      );
      ...
      yield* Effect.forkScoped(
        Effect.forever(
          pullChanges.pipe(
            Effect.flatMap(() => Queue.offer(changeWakeups, undefined)),
          ),
      ...
      yield* Effect.forkScoped(
        Effect.forever(
          Queue.take(changeWakeups).pipe(
            Effect.andThen(
              dependencies.coordinator.getState(dependencies.threadId),
            ),
