...
export function isIgnoredLocalServerProcess(input: DevServerCandidateInput): boolean {
  ...
  const reportedCommandName = path.basename(input.command).toLowerCase();
  ...
  if (
    EXCLUDED_PROCESS_COMMANDS.has(commandName) ||
    EXCLUDED_PROCESS_COMMANDS.has(reportedCommandName)
  ) {
    return true;
  }
