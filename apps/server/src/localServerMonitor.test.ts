...
describe("localServerMonitor", () => {
  ...
  it("keeps dev servers and ignores Electron/Agent Group-style application listeners", () => {
    expect(
      isLikelyDevServerProcess({
        command: "node",
    ...
    expect(
      isIgnoredLocalServerProcess({
        command: "Electron",
        args: "/Applications/Agent Group.app/Contents/MacOS/Agent Group",
        ports: [61449],
      }),
    ).toBe(true);
    expect(
    ...
    expect(
      isLikelyDevServerProcess({
        command: "node",
        args: "node /Users/tester/Developer/agent-group/apps/web/node_modules/.bin/vite",
        ports: [5733],
      }),
    ).toBe(true);
    expect(
  ...
  it("uses parent command lines when a dev-tool child owns the listening port", () => {
    const processInfo = new Map<number, LocalServerProcessInfo>([
      ...
      [
        ...
        {
          ppid: 12064,
          commandLine: "node /Users/tester/Developer/example-site/node_modules/.bin/next dev",
        },
      ],
      [
        12064,
