...
describe("classifyCodexStderrLine", () => {
  ...
  it("classifies text warnings as diagnostics", () => {
    const line =
...
describe("buildCodexProcessEnv", () => {
  it("hydrates the active custom provider env_key from the effective CODEX_HOME", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-codex-env-"));
    try {
      writeFileSync(
  ...
  it("allows the configured desktop browser-use socket in the Codex sandbox", () => {
    const env = buildCodexProcessEnv({
      env: {
        AGENT_GROUP_BROWSER_USE_PIPE_PATH: "/tmp/codex-browser-use/agent-group.sock",
    ...
    expect(env.NODE_REPL_SANDBOX_ALLOWED_UNIX_SOCKETS).toBe(
      "/tmp/existing.sock,/tmp/codex-browser-use/agent-group.sock",
    );
  });
  ...
  it("resolves the browser-use pipe path from desktop env aliases", () => {
    expect(
      resolveCodexBrowserUsePipePath({
        env: { AGENT_GROUP_BROWSER_USE_PIPE_PATH: "/tmp/codex-browser-use/agent-group.sock" },
        platform: "darwin",
      }),
    ).toBe("/tmp/codex-browser-use/agent-group.sock");
  });

  it("applies durable section suppressions inside Agent Group's Codex overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "agent-group-runtime-home-"));
    try {
      writeFileSync(
        path.join(tempDir, "config.toml"),
        [
      ...
      writeFileSync(
        path.join(overlayHome, "agent-group-config-suppressions-v1.json"),
        `${JSON.stringify({
          version: 1,
      ...
      const env = buildCodexProcessEnv({
        env: { AGENT_GROUP_HOME: runtimeHome },
        homePath: tempDir,
        platform: "darwin",
      });
  ...
  it("seeds markerless suppressions for conflicting local browser plugins", () => {
    ...
    try {
      ...
      const suppressionMarker = JSON.parse(
        readFileSync(path.join(overlayHome, "agent-group-config-suppressions-v1.json"), "utf8"),
  ...
  it("repairs stale real files in Agent Group's Codex home overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "agent-group-runtime-home-"));
    try {
  ...
  it("repairs stale auth.json files in Agent Group's Codex home overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "agent-group-runtime-home-"));
    try {
  ...
  it("preserves real generated image directories in Agent Group's Codex home overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "agent-group-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "agent-group-runtime-home-"));
    try {
      writeFileSync(path.join(tempDir, "config.toml"), 'model = "gpt-5.5"', "utf8");
...
describe("handleStdoutLine", () => {
  ...
  it("ignores multiline and standalone JSON leaked from command output", () => {
    ...
    for (const line of ["{", "[", '{"scripts": {', "{}", "[]", '{"name":"agent-group"}']) {
      handleStdoutLine(context, line);
    }
...
describe("startSession", () => {
  ...
  it("uses an isolated scratch workspace path when no cwd is provided", () => {
    ...
    expect(cwd).toContain(`${path.sep}agent-group-codex-workspaces${path.sep}thread-1`);
  });
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Agent Group. Upgrade to v0.37.0 or newer and restart Agent Group.",
        );
      });
