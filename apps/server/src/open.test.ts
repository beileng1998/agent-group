...
import { EDITORS } from "@agent-group/contracts";
...
it.layer(NodeServices.layer)("resolveEditorLaunch", (it) => {
  ...
  it.effect("adds the VS Code URL-handler trailing slash for existing folders", () =>
    Effect.gen(function* () {
      ...
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-vscode-folder-" });
  ...
  it.effect("opens terminal-style editors in the target working directory", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-open-terminal-" });
  ...
  it.effect("falls back to installed macOS app bundles when launchers are absent", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const home = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-open-apps-" });
  ...
  it.effect("prefers the macOS Ghostty app launch even when a ghostty command is on PATH", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const home = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-open-ghostty-" });
...
it.layer(NodeServices.layer)("launchDetached", (it) => {
  ...
  it.effect("rejects when command does not exist", () =>
    Effect.gen(function* () {
      const result = yield* launchDetached({
        command: `agent-group-no-such-command-${Date.now()}`,
        args: [],
      }).pipe(Effect.result);
...
it.layer(NodeServices.layer)("isCommandAvailable", (it) => {
  it.effect("resolves win32 commands with PATHEXT", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-open-test-" });
  ...
  it.effect("uses platform-specific PATH delimiter for platform overrides", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const firstDir = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-open-test-" });
      const secondDir = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-open-test-" });
...
it.layer(NodeServices.layer)("resolveAvailableEditors", (it) => {
  it.effect("returns installed editors for command launches", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-editors-" });
  ...
  it.effect("returns VS Code when the Windows Store package is installed without a CLI", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const programFiles = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-vscode-store-" });
  ...
  it.effect("does not treat Windows app-execution-alias folders as package installs", () =>
    Effect.gen(function* () {
      ...
      const localAppData = yield* fs.makeTempDirectoryScoped({
        prefix: "agent-group-vscode-store-alias-",
      });
  ...
  it.effect("does not treat filesystem-only AppX package directories as installed", () =>
    Effect.gen(function* () {
      ...
      const editor = EDITORS.find((candidate) => candidate.id === "vscode");
      assert.ok(editor);
      const programFiles = yield* fs.makeTempDirectoryScoped({ prefix: "agent-group-vscode-staged-" });
      yield* fs.makeDirectory(
        path.join(
          programFiles,
          "WindowsApps",
