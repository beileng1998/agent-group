import { AuthSessionId, type AuthClientMetadata, type AuthClientSession } from "@agent-group/contracts";
...
export const makeSessionCredentialService = Effect.gen(function* () {
  ...
  const renew: SessionCredentialServiceShape["renew"] = (token, input) =>
    Effect.gen(function* () {
      const verified = yield* verify(token);
      const issuedAt = yield* DateTime.now;
      const expiresAt = DateTime.addDuration(issuedAt, input?.ttl ?? DEFAULT_SESSION_TTL);
      const claims: SessionClaims = {
        v: 1,
      ...
      const encodedPayload = base64UrlEncode(JSON.stringify(claims));
      const signature = signPayload(encodedPayload, signingSecret);
      const updated = yield* authSessions.setExpiresAt({
        sessionId: verified.sessionId,
        expiresAt,
      });
      if (!updated) {
        return yield* toSessionCredentialError("Session token is no longer active.");
      }
      const activeSession = yield* loadActiveSession(verified.sessionId);
      ...
      Effect.mapError((cause) =>
        cause instanceof SessionCredentialError
          ? cause
          : toSessionCredentialError("Failed to renew session credential.", cause),
      ),
    );
