...
export const makeServerAuth = Effect.gen(function* () {
  ...
  const consumeBootstrapCredential = (
    credential: string,
  ): Effect.Effect<BootstrapGrant, AuthError> =>
    config.authToken && credential === config.authToken
      ? DateTime.now.pipe(
          Effect.map((now) => ({
      ...
      : bootstrapCredentials
          .consume(credential)
          .pipe(Effect.mapError(toBootstrapExchangeAuthError));
  ...
  const exchangeBootstrapCredential: ServerAuthShape["exchangeBootstrapCredential"] = (
    ...
    consumeBootstrapCredential(credential).pipe(
