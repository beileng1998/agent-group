      ...
      Layer.effect(
        ...
        Layer.provide(
          ServerConfig.layerTest(process.cwd(), {
            prefix: "agent-group-auth-policy-test-",
          }),
        ),
      ),
    ),
...
describe("ServerAuthPolicyLive", () => {
  it("uses desktop-managed-local policy for loopback desktop mode", async () => {
    const descriptor = await getDescriptor.pipe(
      Effect.provide(
        makeLayer({ mode: "desktop", host: "127.0.0.1", port: 3773, authToken: "secret" }),
      ),
      Effect.scoped,
      Effect.runPromise,
    );
    ...
    expect(descriptor.sessionCookieName).toBe("agent_group_session");
  });

  it("uses remote-reachable policy for wildcard desktop mode", async () => {
    const descriptor = await getDescriptor.pipe(
      Effect.provide(makeLayer({ mode: "desktop", host: "0.0.0.0", authToken: "secret" })),
      Effect.scoped,
      Effect.runPromise,
    );
  ...
  it("uses loopback-browser policy for loopback web mode", async () => {
    const descriptor = await getDescriptor.pipe(
      Effect.provide(makeLayer({ mode: "web", host: "localhost", authToken: "secret" })),
      Effect.scoped,
      Effect.runPromise,
    );
  ...
  it("uses remote-reachable policy for non-loopback web mode", async () => {
    const descriptor = await getDescriptor.pipe(
      Effect.provide(makeLayer({ mode: "web", host: "192.168.1.50", authToken: "secret" })),
      Effect.scoped,
      Effect.runPromise,
    );
  ...
  it("keeps loopback-only no-auth mode explicit and sessionless", async () => {
    const descriptor = await getDescriptor.pipe(
      Effect.provide(makeLayer({ mode: "web", host: "127.0.0.1", authToken: undefined })),
      Effect.scoped,
      Effect.runPromise,
    );
