...
const testLayer = SessionCredentialServiceLive.pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "agent-group-auth-session-test-",
    }),
  ),
...
describe("SessionCredentialServiceLive", () => {
  ...
  it("renews an active browser session without changing its device identity", async () => {
    await Effect.gen(function* () {
      const sessions = yield* SessionCredentialService;
      const issued = yield* sessions.issue({
        ttl: Duration.seconds(2),
        subject: "paired-phone",
        role: "client",
        client: { deviceType: "mobile", os: "Android", browser: "Chrome" },
      });

      yield* TestClock.adjust(Duration.seconds(1));
      const renewed = yield* sessions.renew(issued.token, { ttl: Duration.days(30) });
      yield* TestClock.adjust(Duration.seconds(2));
      ...
      expect(verified.subject).toBe("paired-phone");
      expect(verified.client.deviceType).toBe("mobile");
      expect(active).toHaveLength(1);
      expect(active[0]?.sessionId).toBe(issued.sessionId);
      expect(active[0]?.expiresAt).toEqual(renewed.expiresAt);
    }).pipe(
      Effect.provide(Layer.merge(testLayer, TestClock.layer())),
      Effect.scoped,
      Effect.runPromise,
