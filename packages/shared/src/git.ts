 ...
 */
export const WORKTREE_BRANCH_PREFIX = "agent-group";
...
const AGENT_GROUP_BRANCH_FALLBACK = "update";
...
export function buildAgentGroupBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || AGENT_GROUP_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueAgentGroupBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildAgentGroupBranchName(preferredBranch);
  const existingNames = new Set(existingBranchNames.map((branch) => branch.toLowerCase()));
