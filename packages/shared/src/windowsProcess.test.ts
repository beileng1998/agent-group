...
describe("windowsProcess", () => {
  ...
  it("skips current-directory command hits from where.exe", () => {
    const spawnSync = vi.fn(() => ({
      stdout: [
        "C:\\projects\\agent-group\\codex.cmd",
        "C:\\Users\\test\\AppData\\Roaming\\npm\\codex.cmd",
      ].join("\r\n"),
      status: 0,
    }));
  ...
  it("uses process.cwd for current-directory filtering when cwd is omitted", () => {
    vi.spyOn(process, "cwd").mockReturnValue("C:\\projects\\agent-group");
    const spawnSync = vi.fn(() => ({
      stdout: [
        "C:\\projects\\agent-group\\codex.cmd",
        "C:\\Users\\test\\AppData\\Roaming\\npm\\codex.cmd",
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      "C:\\Windows\\System32\\where.exe",
      ["codex"],
      expect.objectContaining({ cwd: "C:\\projects\\agent-group" }),
    );
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      const root = mkdtempSync(Path.join(tmpdir(), "agent-group-windows-process-"));
