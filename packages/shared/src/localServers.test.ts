...
import type { ServerLocalServerProcess } from "@agent-group/contracts";
...
describe("localServerPrimaryLabel", () => {
  it("prefers the live page title when one was resolved", () => {
    expect(localServerPrimaryLabel(makeServer({ pageTitle: "Agent Group", displayName: "Vite" }))).toBe(
      "Agent Group",
    );
  });
...
describe("localServerFolderLabel", () => {
  it("returns the final segment of a POSIX cwd", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/agent-group-website" }))).toBe(
      "agent-group-website",
    );
  });

  it("ignores a trailing separator", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/agent-group/" }))).toBe(
      "agent-group",
    );
  });
