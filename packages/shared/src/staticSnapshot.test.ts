...
describe("findAsarArchivePath", () => {
  it("resolves the containing archive for a path inside an asar", () => {
    expect(
      findAsarArchivePath(
        "/Applications/Agent Group.app/Contents/Resources/app.asar/apps/server/dist/client",
      ),
    ).toBe(path.join("/Applications/Agent Group.app/Contents/Resources/app.asar"));
  });
  ...
  it("returns null for plain directories", () => {
    expect(findAsarArchivePath("/Users/me/dev/agent-group/apps/web/dist")).toBeNull();
  });
});
