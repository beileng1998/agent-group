...
it.effect("preserves provider and session mentions in thread.turn.start", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeThreadTurnStartCommand({
      type: "thread.turn.start",
      commandId: "cmd-turn-mentions",
      threadId: "thread-1",
      message: {
        messageId: "msg-mentions",
        role: "user",
        text: 'Compare @GitHub with @"Research notes"',
        attachments: [],
        mentions: [
          { name: "GitHub", path: "plugin://github@openai-curated" },
          {
            kind: "session",
            sessionId: "thread-research",
            name: "Research notes",
          },
        ],
      },
    ...
    assert.deepStrictEqual(parsed.message.mentions, [
      { name: "GitHub", path: "plugin://github@openai-curated" },
      {
        kind: "session",
        sessionId: "thread-research",
...
it.effect("accepts the client command that promotes a sidechat", () =>
  Effect.gen(function* () {
    const command = yield* decodeClientOrchestrationCommand({
      type: "thread.sidechat.promote",
      commandId: "cmd-promote-sidechat",
      threadId: "thread-sidechat",
    });

    assert.strictEqual(command.type, "thread.sidechat.promote");
