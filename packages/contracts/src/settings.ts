...
const RemoteAccessHostname = Schema.String.check(
...
export const RemoteAccessServerSettings = Schema.Struct({
  ...
  hostname: RemoteAccessHostname.pipe(Schema.withDecodingDefault(() => "agent-group")),
});
...
const AgentGroupGlobalRules = Schema.String.check(Schema.isMaxLength(1_048_576));

export const AgentGroupServerSettings = Schema.Struct({
  ...
  contextTemplates: Schema.Array(AgentGroupContextTemplate).pipe(
    Schema.withDecodingDefault(() => []),
  ),
  contextEnabled: Schema.Boolean.pipe(Schema.withDecodingDefault(() => true)),
  browserToolsEnabled: Schema.Boolean.pipe(Schema.withDecodingDefault(() => false)),
  globalRules: AgentGroupGlobalRules.pipe(Schema.withDecodingDefault(() => "")),
  promptInstructions: AgentGroupPromptInstructions.pipe(
    Schema.withDecodingDefault(() => ({ ...DEFAULT_AGENT_GROUP_PROMPT_INSTRUCTIONS })),
  ),
});
export type AgentGroupServerSettings = typeof AgentGroupServerSettings.Type;

export const ServerSettings = Schema.Struct({
  enableAssistantStreaming: Schema.Boolean.pipe(Schema.withDecodingDefault(() => true)),
  enableProviderUpdateChecks: Schema.Boolean.pipe(Schema.withDecodingDefault(() => true)),
  /** Kept off until all managed-terminal providers pass the production gate. */
  enableManagedAgentTerminal: Schema.Boolean.pipe(Schema.withDecodingDefault(() => false)),
  ...
  agentGroup: AgentGroupServerSettings.pipe(Schema.withDecodingDefault(() => ({}))),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  enableManagedAgentTerminal: Schema.optionalKey(Schema.Boolean),
  ...
  remoteAccess: Schema.optionalKey(
    Schema.Struct({
      enabled: Schema.optionalKey(Schema.Boolean),
      hostname: Schema.optionalKey(RemoteAccessHostname),
    }),
  ),
  ...
  agentGroup: Schema.optionalKey(
    Schema.Struct({
      defaultModelSelection: Schema.optionalKey(ModelSelectionPatch),
      contextTemplates: Schema.optionalKey(Schema.Array(AgentGroupContextTemplate)),
      contextEnabled: Schema.optionalKey(Schema.Boolean),
      browserToolsEnabled: Schema.optionalKey(Schema.Boolean),
      globalRules: Schema.optionalKey(AgentGroupGlobalRules),
      promptInstructions: Schema.optionalKey(AgentGroupPromptInstructions),
    }),
  ),
});
