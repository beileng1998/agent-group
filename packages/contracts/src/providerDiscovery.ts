...
export const SessionMentionReference = Schema.Struct({
  kind: Schema.Literal("session"),
  sessionId: ThreadId,
  name: TrimmedNonEmptyString,
});
export type SessionMentionReference = typeof SessionMentionReference.Type;

export const MessageMentionReference = Schema.Union([
  ProviderMentionReference,
  SessionMentionReference,
]);
export type MessageMentionReference = typeof MessageMentionReference.Type;
...
// Unified cross-provider skills catalog (Agent Group portable skills). Descriptors use
// `scope` to carry the origin label ("agent-group", "codex", "claude", "cursor", ...).
...
export const ProviderSkillsCatalogResult = Schema.Struct({
  skills: Schema.Array(ProviderSkillDescriptor),
  agentGroupSkillsDir: Schema.optional(TrimmedNonEmptyString),
});
