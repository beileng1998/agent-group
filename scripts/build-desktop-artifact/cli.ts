...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: AGENT_GROUP_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  ...
  mockUpdateServerPort: Flag.string("mock-update-server-port").pipe(
    Flag.withDescription("Mock update server port (env: AGENT_GROUP_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
}).pipe(
