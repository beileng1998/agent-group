...
} from "@agent-group/shared/desktopIdentity";
...
files:
  - url: Agent-Group-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
  - url: Agent-Group-9.9.9-smoke.0-arm64.dmg
    sha512: arm64dmg
    size: 131754935
path: Agent-Group-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
`,
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: Agent-Group-9.9.9-smoke.0-x64.zip
    sha512: x64zip
    size: 132000112
  - url: Agent-Group-9.9.9-smoke.0-x64.dmg
    sha512: x64dmg
    size: 138148807
path: Agent-Group-9.9.9-smoke.0-x64.zip
sha512: x64zip
...
function verifyCanonicalIdentity(): void {
  ...
  if (serverPackage.name !== "@agent-group/server") {
    throw new Error(
      `Expected server package @agent-group/server, got ${serverPackage.name ?? "<missing>"}.`,
    );
  }
  if (serverPackage.private !== true || serverPackage.bin !== undefined) {
    throw new Error(
      "Expected the server workspace package to remain private and expose no binary.",
    );
  }
  if (AGENT_GROUP_PRODUCTION_BUNDLE_ID !== "app.agentgroup.desktop") {
    throw new Error(`Unexpected production bundle ID: ${AGENT_GROUP_PRODUCTION_BUNDLE_ID}.`);
  }
  if (AGENT_GROUP_DESKTOP_UPDATE_CHANNEL !== "agent-group") {
    throw new Error(`Unexpected desktop update channel: ${AGENT_GROUP_DESKTOP_UPDATE_CHANNEL}.`);
  }
  ...
  if (!resolvedPolicy.makeLatest || resolvedPolicy.channel !== AGENT_GROUP_DESKTOP_UPDATE_CHANNEL) {
    throw new Error("Expected Agent Group releases to publish only the Agent Group channel.");
  }
}
...
function verifyReleaseWorkflowSafety(): void {
  ...
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.AGENT_GROUP_FINALIZE_RELEASE == '1'",
  ...
  assertContains(
    workflow,
    "Windows signing is not configured; the artifact will be unsigned.",
    "Expected Windows signing to remain optional.",
  );
  assertContains(
    workflow,
    "sha256sum * > SHA256SUMS.txt",
    "Expected releases to publish SHA-256 checksums.",
  );
  assertNotContains(
    workflow,
    "npm publish",
    "The desktop release workflow must not publish an npm package.",
  );
}
...
const tempRoot = mkdtempSync(join(tmpdir(), "agent-group-release-smoke-"));
...
try {
  ...
  assertContains(
    mergedManifest,
    "Agent-Group-9.9.9-smoke.0-arm64.zip",
    "Merged manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedManifest,
    "Agent-Group-9.9.9-smoke.0-x64.zip",
