...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  it("allows every generated runtime setting through Turbo", () => {
    ...
    for (const name of [
      ...
      "AGENT_GROUP_PORT",
      "AGENT_GROUP_HOME",
      "AGENT_GROUP_NO_BROWSER",
      "AGENT_GROUP_AUTH_TOKEN",
      "AGENT_GROUP_HOST",
      "AGENT_GROUP_LOG_WS_EVENTS",
      "AGENT_GROUP_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      "VITE_WS_URL",
      "VITE_DEV_SERVER_URL",
    ]) {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit AGENT_GROUP_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "AGENT_GROUP_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid AGENT_GROUP_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults development data to the repository-local .agent-group directory", () =>
      Effect.gen(function* () {
        ...
        const expectedHome = resolve(import.meta.dirname, "..", ".agent-group");
        assert.equal(env.AGENT_GROUP_HOME, expectedHome);
      }),
    );

    it.effect("does not inherit desktop runtime configuration", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            AGENT_GROUP_HOME: "/desktop/home",
            AGENT_GROUP_PORT: "3773",
            AGENT_GROUP_HOST: "desktop.internal",
            AGENT_GROUP_AUTH_TOKEN: "desktop-secret",
        ...
        const expectedHome = resolve(import.meta.dirname, "..", ".agent-group");
        assert.equal(env.AGENT_GROUP_HOME, expectedHome);
        assert.equal(env.AGENT_GROUP_PORT, "3775");
        assert.equal(env.VITE_WS_URL, "ws://127.0.0.1:3775");
        assert.equal(env.AGENT_GROUP_HOST, undefined);
        assert.equal(env.AGENT_GROUP_AUTH_TOKEN, undefined);
        assert.equal(env.AGENT_GROUP_NO_BROWSER, undefined);
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          agentGroupHome: "/tmp/custom-agent-group",
          authToken: "secret",
          noBrowser: true,
        ...
        assert.equal(env.AGENT_GROUP_HOME, resolve("/tmp/custom-agent-group"));
        assert.equal(env.AGENT_GROUP_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://127.0.0.1:4222");
        assert.equal(env.AGENT_GROUP_NO_BROWSER, "1");
        assert.equal(env.AGENT_GROUP_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.AGENT_GROUP_LOG_WS_EVENTS, "1");
        assert.equal(env.AGENT_GROUP_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            AGENT_GROUP_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          agentGroupHome: undefined,
        ...
        assert.equal(env.AGENT_GROUP_MODE, "web");
        assert.equal(env.AGENT_GROUP_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.AGENT_GROUP_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses a custom Agent Group home when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
          serverOffset: 0,
          webOffset: 0,
          agentGroupHome: "/tmp/my-agent-group",
          authToken: undefined,
          noBrowser: undefined,
          autoBootstrapProjectFromCwd: undefined,
          logWebSocketEvents: undefined,
        ...
        assert.equal(env.AGENT_GROUP_HOME, resolve("/tmp/my-agent-group"));
      }),
    );
  });
